...
\newcommand\anotherrootofcubedequalsone{\upomega}
...
\begin{equation*}
P(x) = \nfex \Bigl( x - \smalldisplaystyleonehalf \Bigr) \thex Q(x)
\thex .
\end{equation*}
%
Indeed,
...
\vspace{-0.25em}
\begin{equation*}
P(x) = (2x - 1)\bigl(x^2 \nfex - 2\bigr) \nfex = 2 \Bigl(x - \smalldisplaystyleonehalf\Bigr) \nfex \Bigl(x^2 \nfex - 2\Bigr)
\vspace{-0.25em}
\end{equation*}
%
...
It merely replaces the linear factor ${(2x - 1)}$ by its \emph{monic associate} ${\nfex\bigl(x - \nicefrac{1}{2}\bigr)\nthex}$,
...
making the intrinsic symmetries among the roots apparent and explicit, free from the redundancy of a~much larger ambient field.
\end{minipage}
}
...
%
Another familiar example of a~splitting field extension is the passage from~$\mathbb{R}$ to~$\mathbb{C}$.
...
\begin{minipage}{\dimexpr(\linewidth)/8*10\relax}
\indentpar
For the~cubic ${x^3 \nfex - 2}$, the~real root is ${\sqrt[3]{2} \cin \mathbb{R}}$.
Yet, while every polynomial of degree~$3$ has three roots,
the remaining two cannot be discovered in~$\mathbb{R}$ since the~real~line only spans one dimension.
Moving to the complex numbers, all three roots become visible in the complex plane as an~equilateral triangle.
However, the~field~$\mathbb{C}$ is vastly redundant.
It embodies many extraneous elements and algebraic relations
that have nothing to~do with the~algebra of~${x^3 \nfex - 2}$.
The~smallest field embracing exactly what’s needed here is the splitting field ${\mathbb{Q}(\sqrt[3]{2} \thex, \anotherrootofcubedequalsone)}$,
where $\anotherrootofcubedequalsone$ is a~root of~${x^3 \nfex - 1 \nthex = \nfex \bigl(x - 1\bigr) \nthex \bigl(x^2 \nfex + x + 1\bigr)}$ distinct from~$1$,
that is a~root of~${\anotherrootofcubedequalsone^2 \nfex + \anotherrootofcubedequalsone + 1 = 0}$ along with~${\anotherrootofcubedequalsone^3 \nfex = 1}$.
The~third root is the complex conjugate of the~second, ${\overline{\anotherrootofcubedequalsone} \ceq \anotherrootofcubedequalsone^2\nfex}$,
and for every integer~$k$, ${\anotherrootofcubedequalsone^{n+3k}\nfex \ceq \anotherrootofcubedequalsone^n\nfex}$.
In this exact field, the symmetries become apparent and explicit\:---\:%
the triangle of~roots can be rigidly rotated (cyclically permuted)
by~multiplying each root by~${\anotherrootofcubedequalsone}$,
${\sqrt[3]{2} \mapsto \nfex \anotherrootofcubedequalsone\sqrt[3]{2} \mapsto \nfex \anotherrootofcubedequalsone^2\sqrt[3]{2} \mapsto \nfex\nfex \sqrt[3]{2}}$,
or backwards by~${\anotherrootofcubedequalsone^2\nfex}$, %% ${\sqrt[3]{2} \mapsto \nfex \anotherrootofcubedequalsone^2 \sqrt[3]{2}}$
and reflected by complex conjugation.
\end{minipage}
}
...
is precisely what \emph{Galois theory} is about.
\end{minipage}
}
