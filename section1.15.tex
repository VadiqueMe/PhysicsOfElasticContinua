...
\en{such a~basis}\ru{такой базис}\en{ is}\ru{\:---}
\en{not the~only possible one}\ru{не единственно возможный}.
%
\en{Any}\ru{Любые}
\href{https://en.wikipedia.org/wiki/Linear_independence}{\en{linearly independent}\ru{линейно независимые}}
(\en{non\hbox{-}coplanar}\ru{некомпланарные})
\en{vectors}\ru{векторы}~$\bm{a}_i$
\en{can be chosen}\ru{могут быть взяты}
\en{as the~basis ones}\ru{как базисные}.

\begin{wrapfigure}[12]{R}{.48\textwidth}
\makebox[.5\textwidth][c]{
\begin{minipage}[t]{.5\textwidth}

\input{vectorintheobliquebasis.tex}

\vspace{-1.2em}\caption{}\label{fig:ObliqueCoordinates}
\end{minipage}}
\end{wrapfigure}

\en{Then}\ru{Тогда},
\en{some vector}\ru{некий вектор}~$\bm{v}$
\en{can be represented}\ru{может быть представлен}
\en{as a~linear combination}\ru{как линейная комбинация}
\en{of basis vectors}\ru{базисных векторов}~$\bm{a}_i$
\en{with scalar multipliers}\ru{со скалярными множителями}~$v^{i}$
...
%%
\en{The~summation convention}\ru{Соглашение о~суммировании}
\en{gains}\ru{обретает}
\en{the~new conditions}\ru{новые положения}\::
\en{a~summation index}\ru{индекс суммирования}
\en{is repeated}\ru{повторяется}
\en{at the~different levels}\ru{на разных уровнях}
\en{of the~same monomial}\ru{того~же одночлена},
...
${\bm{v} \dotp \bm{a}_i}$
\en{is no longer equal to the~component}\ru{уж\'{е} не~равно компоненте}~$v^{i}$\::
${\bm{v} \dotp \bm{a}_i \hspace{-0.1ex} = \hspace{.1ex} v^{k} \bm{a}_k \hspace{-0.1ex} \dotp \bm{a}_i \neq \vspace{.2ex} v^{i}}$\hbox{\hspace{-0.3ex},}
\en{since}\ru{ведь}
${\bm{a}_i \hspace{-0.1ex} \dotp \bm{a}_k \neq\vspace{.2ex} \delta_{ik}}$.

\en{Fortunately}\ru{К счастью},
\en{for any}\ru{для любых}~${\bm{a}_i}$
\en{there’s another}\ru{есть ещё другая}\:---
\en{the~}\inquotesx{\en{superscript}\ru{надстрочная}}[---]
\en{triple of~vectors}\ru{тройка векторов}~${\bm{a}^{i}}$,
\en{such that}\ru{таких что}
...
\vspace{-0.2em}\noindent
\en{The~triple}\ru{Тройка}~${\bm{a}^{i}}$
\en{is called}\ru{называется}
\en{cobasis}\ru{кобазисом}
\en{or}\ru{или}
\href{https://en.wikipedia.org/wiki/Dual_basis}{\en{reciprocal}\ru{взаимным}~(\en{dual}\ru{дуальным}) \en{basis}\ru{базисом}}.
%
\eqref{fundamentalpropertyofcobasis}
\en{is}\ru{есть}
\en{the~defining}\ru{определяющее}
\en{property}\ru{свойство}
\en{of the~cobasis}\ru{кобазиса}.

\en{The~case}\ru{Случай}\ru{,}
\en{when}\ru{когда}
\en{a~basis}\ru{базис}
\en{coincides}\ru{совпадает}
\en{with its cobasis}\ru{со своим кобазисом}
${\bm{e}^{\hspace{.05ex}i} \hspace{-0.2ex} = \bm{e}_{i}}$\ru{,}
\en{is possible}\ru{возможен}
\en{only when}\ru{только когда}
\en{the basis vectors}\ru{базисные векторы}
\en{are mutually perpendicular}\ru{взаимно перпендикулярны}
\en{to one another}\ru{один другому}
\en{and all}\ru{и~все}
\en{are one unit long}\ru{длиной в~одну единицу}.
%
\en{Such a~basis}\ru{Такой базис}
\en{is called}\ru{называется}
\inquotesx{\en{cartesian}\ru{декартовым (cartesian)}}[.]
...
\en{cobasis vector}\ru{вектор кобазиса}~${\bm{a}^{\hspace{-0.1ex}1}\hspace{-0.1ex}}$
...
\vspace{-0.5em}\noindent
\en{Here}\ru{Здесь},
\en{the~coefficient}\ru{коэффициент}~$\gamma$
\en{turned out to be equal}\ru{получился равным}
(\ru{с~точностью до~знака}\en{up to a~sign})
\en{to the~volume}\ru{объёму}
\en{of~parallelepiped}\ru{параллелепипеда}\ru{,}
\en{built}\ru{построенного}
\en{on~vectors}\ru{на~векторах}~$\bm{a}_i$.
%
\en{The~coincidence that}\ru{Совпадение, что}
\en{in}\ru{в}~\sectionref{section:crossproduct}
\en{the~same volume}\ru{тот~же самый объём}
\en{was presented}\ru{был представлен}
\en{as}\ru{как}~${\hspace{-.22ex}\sqrt{\hspace{-0.36ex}\mathstrut{\textsl{g}}}\hspace{.16ex}}$\ru{,}
\en{is not accidental}\ru{не случайно}.
%
\en{Because}\ru{Ведь}~$\gamma$
\en{equals to}\ru{равен}
\en{the~square root}\ru{квадратному корню}
\en{of}\ru{из}~\href{https://en.wikipedia.org/wiki/Gramian_matrix}{\en{the~gramian}\ru{грамиана}}~\hbox{$\textsl{g} \hspace{.1ex} \hspace{.25ex} \equiv \hspace{.2ex} \determinant \textsl{g}_{i\hspace{-0.1ex}j}$\hspace{-0.12ex}}\:---
\en{the~determinant}\ru{определителя}
\en{of the~symmetric}\ru{симметричной}
\ru{матрицы }\hbox{\href{https://en.wikipedia.org/wiki/J\%C3\%B8rgen_Pedersen_Gram}{J.\,P.\:Gram}\ru{’а}}\en{ matrix}~${\textsl{g}_{i\hspace{-0.1ex}j} \hspace{-0.1ex} \equiv \hspace{.2ex} \bm{a}_i \dotp \bm{a}_j}$.
...
\en{the~derivation}\ru{вывод}\en{ of}~\eqref{theproductoftwopermutations}.
%
...
\vspace{-0.2em}\noindent
\en{or}\ru{или}
\en{by the~columns}\ru{по~колонкам}
...
\vspace{.1em}\noindent
\en{As proven}\ru{Как доказано}
\en{in}\ru{в}~\eqref{determinantofmatrixproduct},
\en{the~determinant}\ru{определитель}
\en{of the~matrix product}\ru{матричного произведения}
\en{of~matrices}\ru{матриц}
\en{is equal}\ru{равен}
\en{to the~multiplication}\ru{перемножению}
\en{of the~determinants}\ru{определителей}
\en{of~each}\ru{каждой}
\en{of~these}\ru{из~этих}
\en{matrices}\ru{матриц},
\en{here}\ru{здесь}~${\permutationsparitysymbols{i\hspace{-0.1ex}j\hspace{-0.1ex}k} \permutationsparitysymbols{pqr}}$.
%
\en{The~elements}\ru{Элементы}
\en{of the~matrix product}\ru{матричного произведения}\ru{\:---}\en{ are}
\en{the~sums}\ru{суммы}
\en{like}\ru{вида}
...
\en{Representing}\ru{Представляя}~${\bm{a}^{\hspace{-0.1ex}1}\hspace{-0.1ex}}$
\en{and the~other}\ru{и~другие}
\en{cobasis vectors}\ru{векторы кобазиса}
\en{as the~sum}\ru{как сумму}
...
\vspace{-0.2em}\noindent
\en{Here}\ru{Здесь}
${e^{i\hspace{-0.1ex}j\hspace{-0.1ex}k}}$ по\hbox{-}прежнему символы чётности перестановки (${\pm 1}$ \en{or}\ru{или}~$0$):
...
любой вектор~(\figureref{fig:DecompositionOfVector}),
но~и найти
...
\en{of the~unit}\ru{единичного}~(\inquotes{\en{metric}\ru{метрического}})
\en{tensor}\ru{тензора}~$\UnitDyad$
...
\vspace{-0.2em}
%%\en{In addition to}\ru{Вдобавок к}~
\en{Besides}\ru{Кроме}~\eqref{fundamentalpropertyofcobasis}
\en{and}\ru{и}~\eqref{basisvectorstocobasisvectors},
\en{there’s one more}\ru{есть ещё один}
\en{way}\ru{способ}
\en{to find}\ru{найти}
\en{the~cobasis vectors}\ru{векторы кобазиса}\:---
\en{via matrix}\ru{через матрицу}~\hbox{$\textsl{g}^{\hspace{.2ex}i\hspace{-0.1ex}j}$\hspace{-0.3ex}},
\en{which is the~inverse}\ru{которая обратна}
\en{of the~}\ru{матрице }Gram\ru{’а}\en{ matrix}~${\textsl{g}_{i\hspace{-0.1ex}j}}$.
