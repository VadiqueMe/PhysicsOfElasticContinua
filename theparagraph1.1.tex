...
\en{waiting for the~reader}\ru{ждущие читателя}
\en{below}\ru{ниже}
\en{in the}\ru{в}~\pararef{para:crossproduct}.}\hbox{\hspace{-0.5ex}.
\par}

\subsection*{ \en{The angle}\ru{Угол}
\en{between two random vectors}\ru{между двумя случайными векторами} }

\en{According to the}\ru{Согласно}~\eqref{anglesbetweenvectorandbasisvectors}
...
\en{for}\ru{для}
\en{the definition}\ru{определения}
\en{of a~vector itself}\ru{самог\'{о} вектора}.
\en{If}\ru{Если}
\en{in each}\ru{в~каждом}
\en{orthonormal}\ru{ортонормальном}
\en{basis}\ru{базисе}~${\bm{e}_i}$
\ru{известна }\en{a~triplet of~components}\ru{тройка компонент}~${v_i}$\en{ is known},
\en{and}\ru{и}
\en{with}\ru{с}
\en{a~rotation}\ru{вращением}
\en{of a~basis}\ru{базиса}
\en{as a~whole}\ru{как целого},
\en{the set of vector components is transformed}\ru{набор компонент вектора преобразуется}
