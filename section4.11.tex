...
\inquotes{\en{Complementary}\ru{Дополнительная}}
\en{work}\ru{работа}~(\en{energy}\ru{энергия})
\en{is named so}\ru{названа так}\ru{,}
\en{as not to be confused}\ru{чтобы не путать}
\en{with }\ru{с~}%
\en{the~}\inquotes{\en{full}\ru{полной}}
\en{work}\ru{работой}
\en{by}\ru{по} Clapeyron\ru{’у}
\eqref{clapeyron:elasticitytheorem} %%(\sectionref{section:theoremsofstatics})
${W \hspace{-0.3ex} = F (\int \hspace{-0.5ex} du) \hspace{-0.2ex} = 2 \hspace{.1ex} \Uppi}$,
\en{where}\ru{где} ${F \narroweq \constant}$.
\par
...
\en{of a~linearly elastic body}\ru{линейно упругого т\'{е}ла}
\en{is}\ru{есть}
