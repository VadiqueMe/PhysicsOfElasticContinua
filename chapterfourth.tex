...
Для решения парадокса в~теории распространена концепция \inquotes{медленного постепенного статического нагружения} (\inquotes{slow gradual application of the load}).
...
{\small
\inquotes{возможная работа сил первого состояния на соответствующих перемещениях, вызванных силами второго состояния, равна возможной работе сил второго состояния на соответствующих перемещениях, вызванных силами первого состояния}
\par}

Для т\'{е}ла с~закреплением на~части поверхности~${o_1}$ рассматриваются две проблемы: первая с~нагрузками $\bm{f}_1$, $\bm{p}_1$ и~вторая с~нагрузками $\bm{f}_2$, $\bm{p}_2$. Словесная формулировка теоремы та~же, что и~в~\chapdotpararef{chapter:genericmechanics}{para:statics}. Метаматическая запись
