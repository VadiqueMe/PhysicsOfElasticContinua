...
\en{In other words, tensor~${^2\!\bm{B}}$ reveals in each basis as matrix of its components~${B_{ij}}$, changing along with a~basis according to~\eqref{orthotransform:2}. Attempts, widespread in literature, to~replace tensors with matrices lead to mistakes if you don’t track the basis to which these matrices correspond.}

\ru{Иными словами, тензор~${^2\!\bm{B}}$ проявляется в~каждом базисе матрицей своих компонент~${B_{ij}}$, меняющейся вместе с~базисом согласно~\eqref{orthotransform:2}. Распространённые в~литературе попытки заменить тензоры матрицами ведут к~ошибкам, если не~следить за~базисом, которому эти матрицы соответствуют.}
