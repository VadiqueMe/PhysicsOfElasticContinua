...
\vspace{-0.2em}\nopagebreak\noindent
\en{that’s why}\ru{вот почему} ${\hspace{-0.15ex}\bm{E}}$ \en{is also called}\ru{также называется} \en{the~unit dyad}\ru{единичной диадой}.

\en{Polyadic representations}\ru{Полиадные представления} \en{like}\ru{типа}~\eqref{polyada:2} \en{help}\ru{помогают} \en{to~operate}\ru{оперировать} \en{with tensors}\ru{с~тензорами} \en{easier}\ru{проще} \en{and }\ru{и~}\en{with }\ru{с~}\en{larger comprehension}\ru{б\'{о}льшим пониманием}:
...
\en{Using}\ru{Используя} \en{decomposition into polyads}\ru{разложение по~полиадам}, \en{it’s easy to~see}\ru{легк\'{о} увидеть} \en{the~actuality}\ru{действительность} \en{of~property}\ru{свойства}~\eqref{identifyofidentitytensor}\:--- \en{the~}\inquotes{\en{unitness}\ru{единичность}} \en{of~tensor}\ru{тензора}~$\bm{E}$:
...
\en{Since}\ru{Поскольку} \en{the~orientation}\ru{ориентация} \en{of~product}\ru{произведения} ${\bm{c} = \bm{a} \times \bm{b}}$ \en{is determined}\ru{определяется}\:--- \en{yet except a~sign}\ru{пока кроме знака}\:--- \en{by multipliers}\ru{сомножителями}, \en{the~sequential triple of~vectors}\ru{последовательная тройка векторов} $\bm{a}$, $\bm{b}$ \en{and}\ru{и}~$\bm{c}$ is what is going to be oriented: \en{one direction out of two}\ru{одно направление из двух} is to be chosen as \inquotesx{\en{positive}\ru{положительное}}[.]

And when vectors are measured using some basis~$\bm{e}_i$ thus represented as linear combinations like ${\bm{a} = a_i \bm{e}_i}$, \en{then}\ru{тогда} \en{the~sequential triple of~basis vectors}\ru{последовательная тройка векторов базиса} $\bm{e}_1$, $\bm{e}_2$, $\bm{e}_3$ is what is going to be oriented.
...
An orientation of the~space is an arbitrary choice of which sequences of~basis vectors are \inquotes{positively} oriented and which are \inquotes{negatively} oriented.
