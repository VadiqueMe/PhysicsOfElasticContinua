...
\en{Don’t be afraid}\ru{Не бойся}: \en{tensors}\ru{тензоры} \en{are used}\ru{используются} \en{just}\ru{просто} \en{because of their wonderful property}\ru{из\hbox{-}за своего чудесного свойства} \en{of invariance}\ru{инвариантности}\:--- \en{independence from coordinate systems}\ru{независимости от систем координат}.
...
\en{It is pretty well characterized}\ru{Он вполне характеризуется} \en{by its length~(modulus, magnitude)}\ru{своей длиной~(модулем, магнитудой)} \en{and its direction in space}\ru{и~своим направлением в~пространстве}.
...
\en{Within such a~\hbox{system}}\ru{В~так\'{о}й \hbox{системе}}\en{,} \hbox{\hspace{-0.2ex}\inquotes{${\dotp\hspace{.22ex}}$}\hspace{-0.2ex}}-\en{products}\ru{произведения} \en{of~\hbox{basis} vectors}\ru{базисных векторов} \en{are equal to}\ru{равны} \en{the~\hbox{Kronecker} delta}\ru{\hbox{дельте} \hbox{Kronecker’а}}:
...
= \hspace{.1ex} \scalebox{0.95}{$\left\{\hspace{.3ex}\begin{gathered}
1, \;i = j \\[-0.25em]
0, \;i \neq j
\end{gathered}\right.$}
\end{equation*}
...
\en{Decomposing vector}\ru{Разлагая вектор}~$\bm{v}$ \en{in ortho\-normal basis}\ru{в~орто\-нормаль\-ном базисе}~${\bm{e}_i}$ (${i = 1, 2, 3}$), \en{we get}\ru{получаем} \en{coefficients}\ru{коэффициенты}~$v_i$\:--- \en{components}\ru{компоненты} \en{of~vector}\ru{вектора}~$\bm{v}$ \en{in that basis}\ru{в~том базисе}~(\figref{fig:componentsofvector})
...
\hspace{.1ex} , \;\;
v_i \hspace{-0.1ex} = \bm{v} \dotp \bm{e}_i
\hspace{.1ex} .
\end{equation}
...
\vspace{-0.1em}\noindent
\en{The~direction of~a~vector in space}\ru{Направление вектора в~пространстве} \en{is measured}\ru{измеряется} \en{by three angles~(co\-sines of~angles)}\ru{тремя углами~(ко\-синусами углов)} \en{between}\ru{между} \en{this~vector}\ru{этим вектором} \en{and~each of basis ones}\ru{и~каждым из базисных}:

\nopagebreak\vspace{-0.22em}\begin{equation}\label{vectorangles}
\operatorname{cos} \measuredangle ({\bm{v}, \bm{e}_i}) \hspace{-0.1ex}
= \scalebox{0.9}{$ \displaystyle\frac{\hspace{.2ex}\raisemath{-0.16em}{\bm{v}}}{\raisemath{-0.05em}{\|\bm{v}\|}} $} \hspace{-0.1ex} \dotp \bm{e}_i \hspace{-0.1ex}
= \hspace{-0.1ex}\scalebox{0.9}{$ \displaystyle\frac{\hspace{.2ex}\raisemath{-0.12em}{v_i}}{\raisemath{-0.1em}{\sqrt{\vphantom{i} v_{\hspace{-0.1ex}j} v_{\hspace{-0.1ex}j}}}} $}
\hspace{.4em}\Leftrightarrow\hspace{.4em}
\tikzmark{beginComponentOfVectorThruLengthAndCosine} v_i \hspace{
.3ex} \tikzmark{endComponentOfVectorThruLengthAndCosine} \hspace{-0.4ex} = \|\bm{v}\| \hspace{-0.1ex} \operatorname{cos} \measuredangle ({\bm{v}, \bm{e}_i})
\hspace{.1ex} .
\end{equation}%
...
\en{However}\ru{Однако}, \en{this is not~enough}\ru{этого м\'{а}ло} (\inquotes{\href{https://en.wikipedia.org/wiki/Necessity_and_sufficiency}{\en{not}\ru{не}~\en{sufficient}\ru{достаточно}}} \en{in jargon}\ru{на жаргоне} \en{of math books}\ru{книг по~математике}) \en{for vector}\ru{для вектора}%%\footnote{\en{There are also pseudovectors}\ru{Бывают ещё и~псевдовекторы}, \en{waiting for the~reader}\ru{ждущие читателя} \en{below}\ru{ниже} \en{in}\ru{в}~\pararef{para:crossproduct+levicivita}.}\hbox{\hspace{-0.5ex}.}
...
\vspace{.1em}\noindent
\en{Appeared}\ru{Возникшие} \en{two\hbox{-}index objects}\ru{двухиндексные объекты} ${\cosinematrix{k'\hspace{-0.1ex}i} \hspace{-0.2ex} \equiv \bm{e}'_{k} \hspace{-0.2ex} \dotp \bm{e}_i}$ \en{and}\ru{и}~${\cosinematrix{ki'} \hspace{-0.25ex} \equiv \bm{e}_k \hspace{-0.15ex} \dotp \bm{e}'_{i}}$ \en{are called}\ru{называются} \inquotes{\en{matrices of~cosines}\ru{матрицы косинусов}} \en{or}\ru{или} \inquotesx{\en{rotation matrices}\ru{матрицы поворота}}[.]

\en{The~}\inquotes{${\dotp\hspace{.25ex}}$}\hbox{\hspace{-0.2ex}-}\en{product}\ru{произведение} (\en{inner product}\ru{внутреннее произведение}, dot product) \en{of two vectors}\ru{двух векторов} \en{is com\-mu\-ta\-tive}\ru{ком\-му\-та\-тивно}\:--- \en{that is}\ru{то есть}, \en{swapping}\ru{обмен местами} \en{multipliers}\ru{множителей} \en{doesn’t change}\ru{не~меняет} \en{the~result}\ru{результат}.
\en{Thus}\ru{Так что}
...
\en{Components}\ru{Компоненты} \en{of the~same vector}\ru{одного и~того~же вектора}~${\bm{v}}$ \en{in}\ru{в}~\en{an~}\inquotes{\en{old}\ru{старом}} \en{and}\ru{и}~\en{a~}\inquotesx{\en{new}\ru{новом}}[---] \en{rotated}\ru{повёрнутом}\:--- \en{orthonormal bases}\ru{ортонормальных базисах}, converting to each other:
...
\en{From}\ru{Из} \en{components}\ru{компонент} \en{of~a~tensor}\ru{тензора}\en{,} \ru{возникает }\en{the~new collection}\ru{новая совокупность}\en{ is emerged} \en{with another}\ru{с~другой} \en{sequence of~indices}\ru{последовательностью индексов}, \en{result’s complexity stays the~same}\ru{сложность результата остаётся той~же}.
...
\en{The~four presented operations}\ru{Четыре представленных действия} \en{are combined}\ru{комбинируются~(сочетаются)} \en{in various sequences}\ru{в~разных последовательностях}.

\en{The~combination}\ru{Комбинация} \en{of~}\en{multiplication}\ru{умножения}~\eqref{tensoraction:multiplication} \en{and}\ru{и}~\en{contraction}\ru{свёртки}~\eqref{tensoraction:contraction}\:--- \en{the~}dot product\:--- \en{is the~most frequent}\ru{самая частая}, в~инвариантной безындексной записи сие обозначается точкой, показывающей свёртку по~соседним индексам:
...
{^\mathrm{n}\hspace{-0.2ex}\bm{a}} \dotp \UnitDyad = \UnitDyad \dotp \hspace{-0.15ex} {^\mathrm{n}\hspace{-0.2ex}\bm{a}} = {^\mathrm{n}\hspace{-0.2ex}\bm{a}} \;\:\:
...
\vspace{-0.1em}\noindent
(\hspace{.25ex}например, для~диад ${\bm{B} \hspace{-0.1ex} = \bm{b}\bm{d}}$ \en{and}\ru{и}~${\bm{Q} \hspace{-0.1ex} = \bm{p}\bm{q}}$
...
\en{Using}\ru{Используя} \en{decomposition into polyads}\ru{разложение по~полиадам}, \en{it’s easy to~see}\ru{легк\'{о} увидеть} \en{the~actuality}\ru{действительность} \en{of~property}\ru{свойства}~\eqref{identifyofidentitytensor}\:--- \en{the~}\inquotes{\en{unitness}\ru{единичность}} \en{of~tensor}\ru{тензора}~$\UnitDyad$:

\nopagebreak\vspace{-0.1em}\[\begin{array}{c}
{^\mathrm{n}\hspace{-0.2ex}\bm{a}} = a_{i\hspace{-0.1ex}j \ldots q} \hspace{.4ex} \bm{e}_i \hspace{.2ex} \bm{e}_j \ldots \bm{e}_q , \;\,  \UnitDyad = \bm{e}_e \bm{e}_e
\\[.4em]
%
...
\en{A~matrix is}\ru{Матрица\:--- это} \en{a~two-dimensional or one-dimensional array of elements}\ru{двумерный или одномерный массив элементов}~(\en{numbers or expressions}\ru{чисел или выражений}).

\en{A~matrix}\ru{Матрица} \en{is}\ru{есть} \en{a~rectangular array}\ru{прямоугольный массив}\:--- \en{it}\ru{она} \en{is made up}\ru{составлена} \en{of rows and columns}\ru{из строк и~столбцов}.
...
The~transpose of a~matrix is computed/performed by swapping columns for rows in that matrix (swapping letters of indices)

...

\begin{otherlanguage}{russian}

... Вводится символ перестановки (permutation symbol) \hbox{O.\hspace{.1ex}Veblen’а}~${e_{i\hspace{-0.1ex}j\hspace{-0.1ex}k}}$\footnote{%
...
\nopagebreak\noindent
\en{the rest are zeroes}\ru{остальные\:--- нули}.
Символ~${e_{i\hspace{-0.1ex}j\hspace{-0.1ex}k}}$~(${\pm 1}$ \en{or}\ru{или}~$0$) меняет знак при~перестановке любых двух индексов, не~меняется при двойной~(\inquotes{круговой}) перестановке, а~при~совпадении какой\hbox{-}либо пары индексов обращается в~нуль.
...
\en{Using}\ru{Используя} $e$\hbox{\hspace{.16ex}-\hspace{.16ex}}\en{symbols}\ru{символы}, \en{the~determinant}\ru{определитель~(детерминант)} \en{of~a~matrix}\ru{матрицы} \en{is expressed as}\ru{выражается как}
...
\en{a~}\hbox{\hspace{-0.2ex}\inquotes{${\hspace{-0.25ex}\times\hspace{-0.1ex}}$}\hspace{-0.2ex}-\en{product}\ru{произведение}}
...
\en{Since}\ru{Поскольку} \en{the~direction in space}\ru{направление в~пространстве} \en{of~product}\ru{произведения} ${\bm{c} = \bm{a} \times \bm{b}}$ \en{is determined}\ru{определяется}\:--- \en{except a~sign}\ru{кроме знака} (\en{an~orientation}\ru{ориентации})\:--- \en{by multipliers}\ru{сомножителями}, \en{the~sequential triple of~vectors}\ru{последовательная тройка векторов} $\bm{a}$, $\bm{b}$ \en{and}\ru{и}~$\bm{c}$ is what is going to be oriented: \en{one direction out of two}\ru{одно направление из двух} is to be chosen as \inquotesx{\en{positive}\ru{положительное}}[.]

\end{minipage}

\vspace{.2em}
And when vectors are measured using some basis~$\bm{e}_i$\:--- being represented as linear combinations like ${\bm{a} = a_i \bm{e}_i}$\:--- \en{then}\ru{тогда} \en{the~sequential triple of~basis vectors}\ru{последовательная тройка векторов базиса} $\bm{e}_1$, $\bm{e}_2$, $\bm{e}_3$ is what is going to be oriented.
Therefore the~sequence of vectors of a~basis becomes significant (for linear combinations like~${a_i \bm{e}_i \hspace{-0.2ex} = \bm{a}}$ it wasn’t).
After orienting, two bases with different sequences of the~same vectors will differ by some permutation.

%\footnote{\textcolor{blue}{A~nonsingular linear mapping is orientation-preserving if the determinant of its matrix is positive.}}

{\small%
The~orientation of a~basis in space is the (kind of) asymmetry that makes a~reflection impossible to replicate by means of a~rotation in that space (without lifting to a~more\hbox{-}dimensional space).
...
An orientation of space is an arbitrary choice of which sequences of~basis vectors are \inquotes{positively} oriented and which are \inquotes{negatively} oriented.
...
\en{This happens}\ru{Это случается} \en{due to the fact that}\ru{из\hbox{-}за того, что} \en{along with changing}\ru{вместе c~изменением} \en{the sign}\ru{знака} \en{of~each}\ru{каждого} \hbox{\hspace{-0.2ex}\inquotes{${\hspace{-0.25ex}\times\hspace{-0.1ex}}$}\hspace{-0.2ex}-\en{product}\ru{произведения}}\:--- \en{which corresponds}\ru{что соответствует} \en{to reflection}\ru{отражению}\:--- \en{the sign}\ru{знак} (\en{and}\ru{и}, \en{accordingly}\ru{соответственно}, \en{the direction}\ru{направление}) \en{of each axial vector}\ru{каждого аксиального вектора} \en{also changes}\ru{также меняется} .
...
\vspace{-0.1em}\noindent
\en{When one of the operands}\ru{Когда один из операндов}\en{ is}\ru{\:---} \en{the }\en{unit}\ru{единичный}~(\en{metric}\ru{метрический}) \en{tensor}\ru{тензор}, \en{from}\ru{из}~\eqref{crossproductforanytwotensors} \en{and}\ru{и}~\eqref{identifyofidentitytensor} ${\forall \, {^\mathrm{n}\hspace{-0.12ex}\bm{\Upsilon}}}$ ${\forall \,\mathrm{n \!>\! 0}}$
...
\vspace{-0.15em}\noindent
\en{And}\ru{И}~\en{such}\ru{такие} \en{identities}\ru{тождества} \en{for any two vectors}\ru{для~любых двух векторов}~$\bm{a}$ \en{and}\ru{и}~$\bm{b}$
...
\end{equation}
\vspace{-1.33em}\[
\scalebox{0.92}{${^2\!\bm{B}} \dotp \bm{a} = \eigenvalue \UnitDyad \dotp \bm{a} \hspace{.1ex},\;\;
...
\vspace{-0.4em}
\en{The roots}\ru{Корни} \en{of characteristic equation}\ru{характеристического уравнения}~\eqref{chardetequation}\:--- \en{eigenvalues}\ru{собственные числа}\:--- не~зависят от~базиса и~потому инвариантны.
...
Соотношение между двумя \inquotes{правыми}~(или двумя \inquotes{левыми}) орто\-нормаль\-ными базисами ${\bm{e}_i}$ и~${\mathcircabove{\bm{e}}_i}$ вполне определено матрицей косинусов~(\pararef{para:vector})
\nopagebreak\vspace{-0.5em}\[
...
\vspace{-0.3em}\noindent
\en{But one may write like this:}\ru{Но кто\hbox{-}то может напис\'{а}ть и~так:}
...
\vspace{-0.32em}\noindent
${\bm{P}}$ \en{is called the rotation tensor}\ru{называется тензором поворота}.
...
\vspace{-0.4em}
Тензор~$\bm{P}$ связывает два вектора\:--- \inquotes{до~поворота}~${\initiallocationvector = \rho_i \mathcircabove{\bm{e}}_i}$ и~\inquotes{после~поворота}~${\locationvector = \rho_i \bm{e}_i}$\:--- с~теми~же компонентами~${\rho_i \hspace{-0.2ex} = \hspace{-0.1ex} \constant}$ у~$\locationvector$ в~актуальном повёрнутом базисе~${\bm{e}_i}$, что~и~у~${\initiallocationvector}$ в~неподвижном базисе~${\mathcircabove{\bm{e}}_i}$~(\inquotes{вектор вращается вместе с~базисом}):
поскольку ${\bm{e}_i = \bm{e}_j \mathcircabove{\bm{e}}_j \hspace{-0.15ex} \dotp \mathcircabove{\bm{e}}_i \:\Leftrightarrow\, \rho_i \bm{e}_i = \bm{e}_j \mathcircabove{\bm{e}}_j \hspace{-0.15ex} \dotp \rho_i \mathcircabove{\bm{e}_i}}$, то

\nopagebreak\vspace{-0.25em}\begin{equation}\label{rodriguesrotationformula}
\locationvector = \bm{P} \dotp\hspace{.2ex} \initiallocationvector
\end{equation}

\vspace{-0.4em}\noindent
(эта связь\:--- обобщённая \href{https://fr.wikipedia.org/wiki/Rotation_vectorielle#Cas_g%C3%A9n%C3%A9ral}{формула поворота Rodrigues’а}).

\vspace{-0.1em}
Поворот~же тензора второй сложности ${\mathcircabove{\bm{C}} = C_{i\hspace{-0.1ex}j} \hspace{.1ex} \mathcircabove{\bm{e}}_i \mathcircabove{\bm{e}}_j}$ в~текущее~(актуальное) положение~${\bm{C} = C_{i\hspace{-0.1ex}j} \hspace{.1ex} \bm{e}_i \bm{e}_j}$ происходит так:
...
\,\;\Leftrightarrow\;\:
\bm{C} = \bm{P} \dotp\hspace{.1ex} \mathcircabove{\bm{C}} \dotp \bm{P}^{\T}
\hspace{-0.32ex} .
\end{equation}
...
\vspace{-0.1em} \noindent
\en{that~is}\ru{то~есть} \en{the~transposed tensor}\ru{транспонированный тензор} \en{coincides}\ru{совпадает} \en{with}\ru{с}~\en{the~reciprocal tensor}\ru{обратным тензором}: ${\bm{P}^{\T} \hspace{-0.32ex} = \bm{P}^{\expminusone} \Leftrightarrow \bm{P} = \bm{P}^{\expminusT}\hspace{-0.25ex}}$.

\begin{otherlanguage}{russian}

Ортогональный тензор не~меняет скалярное произведение векторов, \en{preserving}\ru{сохраняя} \en{lengths and~angles}\ru{дл\'{и}ны и~углы} (\inquotes{\en{metric}\ru{метрику}})

\nopagebreak\vspace{-0.1em}\begin{equation}\label{rotationtensorkeepsmetrics}
\left( \bm{P} \hspace{-0.2ex} \dotp \bm{a}\hspace{.2ex} \right) \dotp \left( \bm{P} \hspace{-0.2ex} \dotp \bm{b}\hspace{.2ex} \right)
= \bm{a} \dotp \bm{P}^{\T} \hspace{-0.4ex} \dotp \bm{P} \hspace{-0.1ex} \dotp \hspace{.1ex} \bm{b}
...
\nopagebreak\vspace{-0.1em}\begin{equation*}
1 = \operatorname{det} \UnitDyad = \operatorname{det} \left({\hspace{-0.1ex} \rotationtensor \dotp \rotationtensor^{\T} \hspace{.2ex}}\right) \hspace{-0.3ex}
...
\vspace{-0.5em}\noindent
Соответствующая собственная ось называется осью поворота; теорема Euler’а о~конечном повороте в~том и~состоит, что такая ось существует.
Если ${\bm{k}}$\:--- орт этой оси, а~${\vartheta}$\:--- величина угла поворота, то тензор поворота представ\'{и}м как

\nopagebreak\vspace{-0.1em}\begin{equation}\label{eulerfiniterotation}
\bm{P}\hspace{.1ex}(\bm{k},\vartheta) = \UnitDyad \operatorname{cos} \vartheta + \bm{k} \times\hspace{-0.2ex} \UnitDyad \operatorname{sin} \vartheta + \bm{k} \bm{k} \left({1 - \operatorname{cos} \vartheta}\right) \hspace{-0.2ex} .
\end{equation}

\vspace{-0.1em}
Доказывается эта формула так. Направление~${\bm{k}}$ при~повороте не~меняется~(${\bm{P} \hspace{-0.2ex}\dotp \bm{k} = \bm{k}\hspace{.12ex}}$), поэтому на~оси поворота ${\mathcircabove{\bm{e}}_3 \hspace{-0.16ex} = \bm{e}_3 \hspace{-0.16ex} = \bm{k}}$. В~перпендикулярной плоскости~(\figref{fig:eulerfiniterotation}) ${\mathcircabove{\bm{e}}_1 \hspace{-0.16ex} = \bm{e}_1 \operatorname{cos} \vartheta - \bm{e}_2 \operatorname{sin} \vartheta}$, ${\mathcircabove{\bm{e}}_2 \hspace{-0.16ex} = \bm{e}_1 \operatorname{sin} \vartheta + \bm{e}_2 \operatorname{cos} \vartheta}$, ${\bm{P} = \bm{e}_i \mathcircabove{\bm{e}}_i \,\Rightarrow\hspace{.2ex}}$~\eqref{eulerfiniterotation}.
...
= \hspace{.4ex} \initiallocationvector \operatorname{cos} \vartheta \hspace{.3ex}+\hspace{.3ex} \bm{k} \times \initiallocationvector \hspace{.2ex} \operatorname{sin} \vartheta \hspace{.3ex}+\hspace{.4ex} \bm{k} \bm{k} \dotp \hspace{.1ex} \initiallocationvector \left({1 - \operatorname{cos} \vartheta}\right) \hspace{-0.25ex}.
\end{equation*}

\vspace{-0.16em}
В~параметрах конечного поворота транспонирование, оно~же обращение, тензора~$\bm{P}$ эквивалентно перемене направления поворота\:--- знака угла~$\vartheta$
\[
\bm{P}^{\T} \hspace{-0.1ex}=\hspace{.1ex} \bm{P} \hspace{.1ex} \bigr|_{\vartheta \,=\hspace{.1ex} -\vartheta} \hspace{-0.1ex} = \UnitDyad \operatorname{cos} \vartheta - \bm{k} \times\hspace{-0.2ex} \UnitDyad \operatorname{sin} \vartheta + \bm{k} \bm{k} \left({1 - \operatorname{cos} \vartheta}\right) \hspace{-0.32ex}.
\]
...
Дифференцируем тождество ортогональности~\eqref{orthogonalityofrotationtensor} по~времени\footnote{Various notations are used to denote the~time derivative.
In~addition to the Leibniz’s notation ${\scalebox{.9}{$ \raisemath{.3em}{dx} $} \hspace{-0.3ex} / \hspace{-0.4ex} \scalebox{.9}{$ \raisemath{-.3em}{dt} $}\hspace{.1ex}}$, the~very popular short\hbox{-}hand one is the~\inquotes{over-dot} Newton’s notation ${\mathdotabove{x}}$.}
...
Поэтому согласно~\eqref{companionvector} он представ\'{и}м сопутствующим вектором как ${\mathdotabove{\bm{P}} \dotp \bm{P}^{\T\!} = \bm{\omega} \times \UnitDyad = \bm{\omega} \times \bm{P} \dotp \bm{P}^{\T}}$\!.
То~есть
...
\tikzmark{StartBracePa} {\bm{k} \hspace{-0.24ex}\times\hspace{-0.4ex} \UnitDyad \operatorname{sin} \vartheta \hspace{.2ex}} \tikzmark{EndBracePa} {} \hspace{.16ex}\right)^{\hspace{-0.2ex}\tikz[baseline=-0.5ex]\draw[black, fill=black] (0,0) circle (.266ex);} \hspace{-0.05em} = \\[0.4em]
%
...
Bivalent unit tensor~(identity tensor,  metric tensor)~${\hspace{-0.1ex}\UnitDyad}$,
the~one which is neutral~\eqref{identifyofidentitytensor} to the~dot product operation,
...
\vspace{-0.2em} \noindent --- \en{here’s no~need}\ru{тут нет нужды} \en{to~expand}\ru{разворачивать} \en{vectors}\ru{векторы}~$\bm{a}$ \en{and}\ru{и}~$\bm{b}$, \en{expanding just}\ru{развернув лишь} \en{differential operator}\ru{дифференциальный оператор}~${\hspace{-0.13ex}\boldnabla}$.
...
Представим теперь, что функция~${\locationvector(q^{\hspace{.1ex}k})}$ не~известна, но~\hbox{зат\'{о}} в~каждой точке пространства определены шесть независимых элементов положительной симметричной метрической матрицы Грама~${\textsl{g}_{i\hspace{-0.1ex}j}(q^{\hspace{.1ex}k})}$.
...
символы Христоффеля это \inquotes{\en{components of~connection}\ru{компоненты связности}} \en{in local coordinates}\ru{в~локальных координатах}
...
\inquotes{\en{The direct approach}\ru{Прямой подход}} \en{to~tensors}\ru{к~тензорам} \en{is explicated}\ru{излагается}, \en{for instance}\ru{например}, \en{in the appendices}\ru{в~приложениях} \en{of books}\ru{книг} \en{by }\foreignlanguage{russian}{А.\,И.\;Лурье} (A.\,I.\;Lurie)~\cite{lurie-nonlinearelasticity, lurie-theoryofelasticity}.
...
\en{Information about tensor calculus}\ru{Информация о~тензорном исчислении}\en{ is}\ru{\:---} \en{also}\ru{также} \en{a~piece of}\ru{часть}
\en{especial and profound}\ru{своеобр\'{а}зной и~глубокой} \ru{книги }C.\:Truesdell’\en{s}\ru{а}\en{ book}~\cite{truesdell-firstcourse}.
