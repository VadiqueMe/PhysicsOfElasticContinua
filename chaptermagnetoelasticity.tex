...
\en{Problems}{Проблемы},
\en{which}\ru{которые}
\en{are solved}\ru{решаются}
\en{using the models}\ru{используя модели}
\en{of an~elastic continua}\ru{упругих \rucontinuum{}ов}, \en{usually}\ru{обыкновенно}
\en{include}\ru{включают}
\en{the already known}\ru{уж\'{е} известные}
\en{external loads}\ru{внешние нагрузки}
(\en{how to~find them}\ru{как найти их}
\en{is}\ru{это}
\en{a~distinct problem}\ru{отдельная проблема}).
%
\en{When}\ru{Когда}
\en{electric currents}\ru{электрические токи}
\en{flow}\ru{текут}
\en{in a~body}\ru{в~теле},
\en{the~magnetic field}\ru{магнитное поле}
\en{creates}\ru{создаёт}
\en{a~load}\ru{нагрузку}.
%
\en{When}\ru{Когда}
\en{a~body}\ru{тело}
\en{is deformed}\ru{деформируется},
\en{the~magnetic field changes too}\ru{меняется и~магнитное поле}.
\en{If}\ru{Если}
\en{the~field’s sensitivity}\ru{чувствительность п\'{о}ля}
\en{to a~deformation}\ru{к~деформации}
\en{is high}\ru{высок\'{а}},
\en{then}\ru{тогда}
\en{we need}\ru{нам нужно}
\en{to solve}\ru{решить}
\en{a~problem}\ru{проблему}
\en{of an~elasticity}\ru{упругости}
\en{along}\ru{вместе}
\en{with a~magnetism}\ru{с~магнетизмом}.
...
\begin{align*}
\eqrefwithchapterdotpara{divergenceofdyadicproducoftwovectors}{chapter:mathapparatus}{para:spatialdifferentiationoftensorfields}
...
\\[-0.1em]
%
\eqrefwithchapterdotpara{gradientofdotproductoftwovectors}{chapter:mathapparatus}{para:spatialdifferentiationoftensorfields}
& \hspace{.27em}\Rightarrow\hspace{.32em}
