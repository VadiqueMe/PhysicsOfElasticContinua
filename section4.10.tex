...
\label{section:principleofminimumpotentialenergy}
...
(
   ...
   \en{or else}\ru{иначе} ${ \smash{ \potentialenergydensity (\hspace{-0.1ex} \bm{u} \hspace{-0.1ex} )}\hspace{-0.1ex} }$
...
\hspace{-0.1ex} = \hspace{-0.1ex} \potentialenergyfunctional_{\hspace{-0.1ex}\text{min}}
\hspace{.1ex},
\end{equation*}
\en{which}\ru{которая}
\en{according to}\ru{согласно}
\en{the }\ru{теореме }Clapeyron’\en{s}\ru{а}\en{ theorem}~\eqref{clapeyron:elasticitytheorem}
\en{is}\ru{есть}
...
\en{and}\ru{и}~${{=} \hspace{.33ex} \zerovector}$ \en{elsewhere}\ru{где-либо ещё} \en{on}\ru{на}~${o(\boundary \mathcal{V})}$.
${\linearstress \hspace{-0.1ex} = \hspace{-0.3ex} \boldnabla \bm{u} \dotdotp \stiffnesstensor \hspace{-0.1ex} = \hspace{-0.1ex} \constantbivalent}$ along with constant~$\bm{p}$ \en{and}\ru{и}~$\volumeloadvector$\hbox{\hspace{-0.2ex}.}
...
\textcolor{magenta}{\russianlanguage{Разность преобразуется до}}
...
\en{is positive definite}\ru{положительно определён} (\sectionref{section:uniquenessfordynamicproblem})
