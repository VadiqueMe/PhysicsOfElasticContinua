...
= \hspace{.1ex} \| \bm{a} \| \hspace{.1em} \| \bm{b} \| \hspace{.1em} \sine \measuredangle (\bm{a}, \bm{b})
\hspace{.1ex} .
\end{equation*}
...
(\en{except when}\ru{кроме случаев, когда}
\en{a~vector}\ru{вектор}
...
\en{twice}\ru{дважды}\:---
\en{the~second time}\ru{второй раз}
...
%
(\en{And}\ru{А}~\en{mirroring}\ru{зеркалирование}
\en{changes}\ru{меняет}
\en{the~sign}\ru{знак}
\en{of the~}\crossproductinquotes\hbox{-}\en{product}\ru{произведения}.)
...
\en{from}\ru{из}
\en{the~kinematics}\ru{кинематики}
\en{of an~absolutely rigid}\ru{абсолютно жёсткого}~(\en{undeformable}\ru{недеформируемого})
\en{body}\ru{тела}
...
= \pm \hspace{.4ex}
\bm{e}_i \hspace{-0.36ex} \times \hspace{-0.2ex} \bm{e}_j \hspace{-0.15ex} \dotp \hspace{.1ex} \bm{e}_k }$

${ e_{pqr} \hspace{-0.2ex}
= \pm \hspace{.4ex}
\bm{e}_p \hspace{-0.36ex} \times \hspace{-0.2ex} \bm{e}_q \hspace{-0.15ex} \dotp \hspace{.1ex} \bm{e}_r }$

\en{with}\ru{с}~\inquotes{$-$}
\en{for}\ru{для}
\inquotes{\en{left}\ru{левой}}
\en{triple}\ru{тройки}
...
\nopagebreak\vspace{-0.2em}
\begin{equation}\label{thedirectrelationoftheunitdyadwiththepermutationsparity}
