...
\en{Mention}\ru{Упоминание} \en{of~tensors}\ru{тензоров} \en{may scare away the~reader}\ru{может отпугнуть читателя}, \en{commonly avoiding needless complications}\ru{обычно избегающего ненужных сложностей}.
...
\en{Here it is}\ru{Вот он}\:--- \en{vector}\ru{вектор}~$\bm{v}$.
\en{It is pretty well characterized}\ru{Он вполне характеризуется} \en{by }\en{its length}\ru{своей длиной}~(\en{magnitude}\ru{магнитудой}, \en{modulus}\ru{модулем}) \en{and }\ru{и~}\en{its direction}\ru{своим направлением} \en{in space}\ru{в~пространстве}.
\en{It objectively exists by itself}\ru{Он объективно существует сам по себе}, \en{independent}\ru{независимо} \en{of}\ru{от} \en{methods}\ru{методов} \en{and}\ru{и}~\en{units}\ru{единиц} \en{of~measurements}\ru{измерений} \en{of~both lengths and directions}\ru{и~длин, и~направлений}.
...
\hfill\hbox{\en{for any}\ru{для любого} \inquotes{\en{orthonormal}\ru{ортонормального}} \en{basis}\ru{базиса}}.
...
\equiv \hspace{-0.1ex} \scalebox{0.84}{$\displaystyle\sum_{i=1}^3$} \hspace{.2ex} {v_i \bm{e}_i} \hspace{-0.1ex}
...
\nopagebreak\vspace{-0.2em}\begin{equation}\label{vectorangles}
\operatorname{cos} \measuredangle ({\bm{v}, \bm{e}_i}) \hspace{-0.1ex}
...
\en{The cosine of the angle}\ru{Косинус угла} \en{between two vectors}\ru{между двумя векторами} is the same as the inner~(dot) product of these vectors when they are normalized to both have the magnitude equal to the one unit of length
...
For this purpose, use unit vectors, which are vectors with a~magnitude of 1.

\emph{\en{The angle}\ru{Угол} \en{between two random vectors}\ru{между двумя случайными векторами}.}
\en{According to}\ru{Согласно}~\eqref{vectorangles}
...
%%\footnote{\en{There are also pseudovectors}\ru{Бывают ещё и~псевдовекторы}, \en{waiting for the~reader}\ru{ждущие читателя} \en{below}\ru{ниже} \en{in}\ru{в}~\pararef{para:crossproduct+levicivita}.}\hbox{\hspace{-0.5ex}.}
...
\vspace{-0.1em}
\en{A~triple}\ru{Тройка} \en{of mutually perpendicular}\ru{взаимно перпендикулярных} \en{unit vectors}\ru{единичных векторов} \en{can only rotate}\ru{может только поворачиваться} \en{and thereby}\ru{и~тем самым} \en{characterize}\ru{характеризовать} \en{the angular orientation}\ru{угловую ориентацию}.
...
\vspace{.1em}\noindent
\en{Appeared here}\ru{Возникшие тут} \en{two\hbox{-}index objects}\ru{двухиндексные объекты}
...
\en{are used to shorten formulas}\ru{используются для укорочения формул}.

%%\en{are called}\ru{называются} \inquotes{\en{matrices of~cosines}\ru{матрицы косинусов}} \en{or}\ru{или} \inquotesx{\en{rotation matrices}\ru{матрицы поворота}}[.]

\en{The~}\inquotes{${\dotp\hspace{.25ex}}$}\hbox{\hspace{-0.2ex}-}\en{product}\ru{произведение} (dot product, \en{inner product}\ru{внутреннее произведение}) \en{of two vectors}\ru{двух векторов} \en{is commutative}\ru{коммутативно}\:--- \en{that is}\ru{то есть}, \en{swapping}\ru{обмен местами} \en{multipliers}\ru{множителей} \en{doesn’t change}\ru{не~меняет} \en{the~result}\ru{результат}.
\en{Thus}\ru{Так что}
...
\vspace{-0.1em}
\en{If}\ru{Если} ${f_{\hspace{-0.1ex}i\hspace{-0.1ex}j} \hspace{-0.2ex} = \delta_{i\hspace{-0.1ex}j}}$, \en{then}\ru{то} ${\digamma \hspace{-0.4ex} = \delta_{i\hspace{-0.1ex}j} \hspace{.1ex} a_i b_{\hspace{-0.1ex}j} \hspace{-0.2ex} = a_i b_i}$\:--- \en{the~}\inquotes{${\dotp\hspace{.25ex}}$}\hbox{\hspace{-0.2ex}-}\en{product}\ru{произведение} (dot product, \en{inner product}\ru{внутреннее произведение}, \en{scalar product}\ru{скалярное произведение}) \en{of~two vectors}\ru{двух векторов}.
...
\en{In the direct indexless notation}\ru{В~прямой безиндексной записи} \en{this is denoted by a~large dot}\ru{это обозначается крупной точкой}, \en{which}\ru{которая} \en{shows}\ru{показывает} \en{a~contraction}\ru{свёртку} \en{by adjacent indices}\ru{по~соседним индексам}:
...
\en{The following}\ru{Следующее} \en{identity}\ru{тождество} \en{shows}\ru{показывает}\ru{,} \en{how to swap multipliers}\ru{как обменять местами множители} \en{for}\ru{для} \en{the }\hbox{\hspace{-0.2ex}\inquotes{${\dotp\hspace{.22ex}}$}\hspace{-0.2ex}-\en{product}\ru{произведения}} (dot product\ru{’а}) \en{of the two second complexity tensors}\ru{двух тензоров второй сложности}

\nopagebreak\vspace{-0.2em}\begin{equation}\label{transposeofdotproduct}
\begin{array}{c}
...
\bm{B} \dotp \bm{c} = \bm{c} \hspace{.2ex} \dotp \bm{B}^{\T}
\hspace{-0.3ex} .
\end{equation}
...
\en{A~matrix has}\ru{У~матрицы} \en{the same number of elements in each row}\ru{одно и то~же число элементов в~каждой строке} \en{and}\ru{и} \en{the same number of elements in each column}\ru{одно и то~же число элементов в~каждом столбце}.

\en{The matrix algebra}\ru{Матричная алгебра} \en{includes}\ru{включает}  \en{linear operations}\ru{линейные операции}\:--- \en{addition of matrices}\ru{сложение матриц} \en{and}\ru{и} \en{scalar multiplication}\ru{скалярное умножение} (\en{multiplication of a~matrix by a~scalar}\ru{умножение матрицы на скаляр}).
\en{Addition of matrices}\ru{Сложение матриц} \en{is possible}\ru{возможно} \en{only when}\ru{только когда} \en{both addends}\ru{оба слагаемых} \en{have the same dimensions}\ru{имеют одинаковые размеры}.
...
\en{Symbol}\ru{Символ}~${e_{i\hspace{-0.1ex}j\hspace{-0.1ex}k}}$~(${\pm 1}$ \en{or}\ru{или}~$0$) \en{changes its sign}\ru{меняет свой знак} \en{with permutation}\ru{с~перестановкой} \en{of any two indices}\ru{любых двух индексов}, \en{it does not change}\ru{он не~меняется} \en{with double}\ru{с~двойной}~(\inquotes{\en{circular}\ru{круговой}}) \en{permutation}\ru{перестановкой}\en{,} \en{and}\ru{и}~\en{it nullifies}\ru{он обнуляется}\ru{,} \en{if}\ru{если} \en{any index is repeated}\ru{какой-либо индекс повторяется}.
...
${
\underset{\raisebox{.12em}{\scalebox{.7}{$i$,$\hspace{.15ex}j$}}}{\operatorname{det}} \hspace{.4ex} \delta_{i\hspace{-0.1ex}j} \hspace{-0.15ex} = 1
}$
...
\inquotes{\en{Determinant of the product is equal to the product of determinants}\ru{Определитель произведения равен произведению определителей}}
...
\[
e_{\hspace{-0.25ex}f\hspace{-0.2ex}gh} \hspace{.33ex} \underset{\raisebox{.15em}{\scalebox{0.7}{$m$,$n$}}}{\operatorname{det}} \, B_{m\mathcolor{blue}{s}} \hspace{.1ex} C_{\mathcolor{blue}{s}n} \hspace{-0.2ex}
...
\en{For basis}\ru{Для базиса}~${\bm{e}_i}$ \en{of mutually perpendicular one\hbox{-}unit long vectors}\ru{из взаимно перпендикулярных векторов единичной длины}
...
\smalldisplaystyleonehalf \hspace{-0.1ex} \left( \bm{c}\bm{d} + \hspace{-0.1ex} \bm{d}\bm{c} \hspace{.1ex} \right)
- \hspace{.16ex} \smalldisplaystyleonehalf \hspace{-0.1ex} \left( \bm{c} \hspace{-0.1ex} \times \hspace{-0.2ex} \bm{d} \hspace{.2ex} \right) \hspace{-0.12ex} \times \hspace{-0.25ex} \UnitDyad
\hspace{.1ex} .
\end{equation}

\end{otherlanguage}

\en{\section{Eigenvectors and eigenvalues of a~tensor}}
...
\end{equation}
\vspace{-1.33em}\[
\scalebox{0.92}{${^2\!\bm{B}} \dotp \bm{a} = \eigenvalue \UnitDyad \dotp \bm{a}
\hspace{.1ex},\;\;
...
${( B_{i\hspace{-0.1ex}j} \hspace{-0.12ex} - \eigenvalue \delta_{i\hspace{-0.1ex}j} ) \hspace{.25ex} a_j \hspace{-0.1ex} = 0}$ \en{has}\ru{имеет} \en{a~non-zero solution}\ru{ненулевое решение}\ru{,} \en{if}\ru{если} \en{the determinant}\ru{определитель} \en{of a~matrix of~components}\ru{матрицы компонент} ${\smash{\underset{\raisebox{.15em}{\scalebox{0.7}{$i$,$\hspace{.15ex}j$}}}{\operatorname{det}}} \hspace{.32ex} ( B_{i\hspace{-0.1ex}j} \hspace{-0.12ex} - \eigenvalue \delta_{i\hspace{-0.1ex}j} )}$ \en{is equal to zero}\ru{равен нулю}:
...
\\[.1em]
%
\mathrm{II} & = & \scalebox{0.92}[0.96]{$B_{1\hspace{-0.1ex}1}B_{22} \hspace{-0.2ex} - \hspace{-0.1ex} B_{12}B_{21} \hspace{-0.2ex} + \hspace{-0.1ex} B_{1\hspace{-0.1ex}1}B_{33} \hspace{-0.2ex} - \hspace{-0.1ex} B_{13}B_{31} \hspace{-0.2ex} + \hspace{-0.1ex} B_{22}B_{33} \hspace{-0.2ex} - \hspace{-0.1ex} B_{23}B_{32}$}
\hspace{.1ex} ,
\\[.1em]
%
...
\right]$} \hspace{-0.5ex}
= \smalldisplaystyleonehalf \scalebox{0.84}{$\left[
...
\indent 1$^{\circ}$\hspace{-1ex}.\, \en{The eigenvalues}\ru{Собственные значения} \en{of a~symmetric tensor}\ru{симметричного тензора}\ru{\:---}\en{ are} \en{real numbers}\ru{действительные числа}.\\
\indent 2$^{\circ}$\hspace{-1ex}.\, \en{The characteristic axes~(directions)}\ru{Собственные оси~(направления)} \en{for different eigenvalues}\ru{для различных собственных значений} \en{are orthogonal}\ru{ортогональны} \en{to each other}\ru{друг другу}.
...
\vspace{-0.3em}\noindent
\en{Also}\ru{Также}, \en{a~rotation}\ru{поворот} \en{of a~tensor}\ru{тензора} \en{can be described}\ru{может быть описан} \en{via}\ru{через} \en{another tensor}\ru{другой тензор}:
...
\vspace{-0.3em}\noindent
\en{called}\ru{называемый} \en{the rotation tensor}\ru{тензором поворота}.

\en{Components of}\ru{Компоненты}~$\rotationtensor$ \en{both }\ru{и~в~начальном}\en{in initial}~${\mathcircabove{\bm{e}}_i}$\ru{,} \en{and}\ru{и}~\en{in rotated}\ru{в~повёрнутом}~${\bm{e}_i}$ \en{bases}\ru{базисах} \en{are the same}\ru{одни и~те~же}
...
\en{Tensor}\ru{Тензор}~$\rotationtensor$ \en{relates}\ru{соотносит} \en{two vectors}\ru{два вектора}\:--- \inquotes{\en{before rotation}\ru{до~поворота}}~${\initiallocationvector = \hspace{-0.1ex} \rho_i \hspace{.1ex} \mathcircabove{\bm{e}}_i}$ \en{and}\ru{и}~\inquotes{\en{after rotation}\ru{после поворота}}~${\locationvector = \hspace{-0.1ex} \rho_i \hspace{.1ex} \bm{e}_i}$.
...
\vspace{-0.1em} \noindent
\en{that~is}\ru{то~есть} \en{the~transposed tensor}\ru{транспонированный тензор} \en{coincides}\ru{совпадает} \en{with}\ru{с}~\en{the~reciprocal tensor}\ru{обратным тензором}: ${\rotationtensor^{\T} \hspace{-0.32ex} = \rotationtensor^{\expminusone} \Leftrightarrow \rotationtensor = \rotationtensor^{\expminusT}\hspace{-0.25ex}}$.
...
Тензор поворота это ортогональный тензор \en{with}\ru{с}~${\operatorname{det} \rotationtensor \hspace{-0.1ex} = 1}$.
Но не~только лишь тензоры поворота обладают свойством ортогональности.
\en{When}\ru{Когда} \en{in}\ru{в}~\eqref{introductionofrotationtensor} один из базисов \inquotesx{правый}[,] а~другой \inquotesx{левый}[,] \en{then it’s}\ru{тогда это} \en{combination of a~rotation and a~reflection}\ru{комбинация поворота и~отражения} (\inquotes{rotoreflection}) ${\rotationtensor = -\UnitDyad \dotp \rotationtensor}$ \en{with}\ru{с}~${\operatorname{det} \left( -\UnitDyad \dotp \rotationtensor \hspace{.16ex} \right) \hspace{-0.1ex} = -1}$.
...
\nopagebreak\vspace{1em}\begin{equation*}\begin{array}{c}
\rotationtensor \dotp \bm{a} = \eigenvalue \bm{a} \:\Rightarrow\:
...
\en{\section{Tensor fields. Spatial differentiation}}

\ru{\section{Тензорные поля. Пространственное дифференцирование}}

\label{para:spatialdifferentiationoftensorfields}
...
\noindent
Пусть в~каждой точке некоторой области трёхмерного пространства определена величина~$\varsigma$.
Тогда имеем поле~${\varsigma \!=\! \varsigma(\locationvector)}$, \en{where}\ru{где}~$\locationvector$\en{ is}\ru{\:---} \en{location vector}\ru{вектор положения}~(\en{radius vector}\ru{вектор\hbox{-}радиус}) \en{of~a~point}\ru{точки} \en{in~space}\ru{пространства}.
...
Therefore, rectangular coordinates \en{of~any point}\ru{любой точки} \en{can be converted}\ru{могут быть преобразованы} to its curvilinear coordinates \en{and}\ru{и}~\en{vice versa}\ru{обратно}.

Commonly used sets of~coordinates are rectangular (\inquotes{cartesian}), spherical and cylindrical.
