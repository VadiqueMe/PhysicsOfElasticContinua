...
\textcolor{magenta}{Vectors are linear (straight), they cannot be curved.}

\textcolor{magenta}{Vectors are neither straight nor curved.}
...
A~vector has the~magnitude and the~direction. It can’t be thought of as a~line.

\subsection{\en{Number line}\ru{Числовая прямая}}

\subsection{\en{Plane and space}\ru{Плоскость и пространство}}

\subsection{\en{Differentiation into small differential chunks}\ru{Дифференциация на м\'{а}лые дифференциальные кусочки}}
...
Multiplying a~vector by negative one reverses the direction but doesn’t change the magnitude.

For every action, there’s an equal (in magnitude) and opposite (in direction) reaction force.

A~vector may be also represented as the sum (combination) of some trio (called \inquotes{basis}) of~other vectors scaled by numbers (coefficients).
...
For this purpose, we use unit vectors, which are vectors with a~magnitude of 1.
...
Therefore, a rotation matrix is not really a “rotation” matrix; it is an orientation matrix. 
\par}
