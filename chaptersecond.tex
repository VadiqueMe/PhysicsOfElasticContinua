...
\en{\lettrine[lines=2, findent=2pt, nindent=0pt]{I}{n} generic mechanics, collections of~particles (\inquotesx{pointlike masses}[,] \inquotes{material points}) are considered. In a~collection of~$N$ particles each particle has its mass~${m_k \hspace{-0.2ex}=\hspace{-0.1ex} \const}$ and motion function~${\bm{r}_k(t)}$. Function~${\bm{r}_k(t)}$ is measured relative to some reference system\:--- a~solid body with a~clock~(\figref{fig:referencesystem}).}

\ru{\lettrine[lines=2, findent=2pt, nindent=0pt]{В}{\hspace{-0.25ex}} общей механике рассматриваются совокупности частиц (\inquotesx{точеч\-ных масс}[,] \inquotes{материальных точек}). В~совокупности $N$~\hbox{частиц} каждая частица имеет свою массу~${m_k \hspace{-0.2ex}=\hspace{-0.1ex} \const}$ и~функцию движения~${\bm{r}_k(t)}$. Функция~${\bm{r}_k(t)}$ измеряется отно\-си\-тель\-но н\'{е}которой системы отсчёта\:--- твёрдого тела с~часами~(\figref{fig:referencesystem}).}
...
\bm{F}_k \hspace{-0.1ex} = \bm{F}^{\expexternal}_{\hspace{-0.16ex}k}
 + \scalebox{0.88}{$\displaystyle \underset{\raisemath{.25ex}{\smash{j}}}{\sum}$} \hspace{.2ex} \bm{F}^{\expinternal}_{\hspace{-0.16ex}kj} \hspace{-0.12ex},
\end{equation}
...
\en{Transition from formulas}\ru{Переход от~формул}, \en{containing}\ru{содержащих} \en{summation over discrete points}\ru{суммирование по~дискретным точкам}, \en{to formulas for a~continuous body}\ru{к~формулам для сплошного тела} \en{is made}\ru{осуществляется} \en{by replacing}\ru{заменой} \en{masses of~particles}\ru{масс частиц} \en{with mass}\ru{на~массу} ${dm \hspace{-0.1ex} = \hspace{-0.15ex} \rho \hspace{.2ex} d\mathcal{V}}$ \en{of~volume element}\ru{элемента объёма}~${d\mathcal{V}}$~($\rho$\ru{\:---}\en{~is}~\en{mass density}\ru{плотность массы}), \en{and}\ru{и}~\en{integration over the~whole volume of a~body}\ru{интегрированием по~всему объёму тела}.
...
\en{For an~absolutely rigid body}\ru{Для совершенно жёсткого тела}, \en{components}\ru{компоненты}~${x_i}$ \en{don’t depend on~time}\ru{не~зависят от~времени}~(${x_i \hspace{-0.16ex} = \const}$) \en{and}\ru{и}~${\mathdotabove{\bm{x}} = x_i \hspace{.1ex} \mathdotabove{\bm{e}}_i}$
