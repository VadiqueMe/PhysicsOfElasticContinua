...
\en{A~reference system~(or \inquotes{reference frame})}\ru{Система отсчёта}
...
\en{the~units of measurements}\ru{единицы измерения},
%
\item
\en{a~clock}\ru{часы}.
\end{itemize}
...
\en{was}\ru{было}
\en{some absolute space}\ru{некое абсолютное пространство}:
\en{empty at~first}\ru{сначала пустое},
\en{and then}\ru{а~затем}
\en{filled with the~continuous elastic medium}\ru{заполненное сплошной упругой \hbox{средой}}\:---
\en{the~æther}\ru{эфиром~(æther)}.
% the ether is a mechanical model
\en{Later it became clear}\ru{Позже стало ясно}\ru{,}
\en{that}\ru{что}
\en{for classical mechanics}\ru{для классической механики}
\ru{могут быть использованы }\en{any reference systems}\ru{любые системы отсчёта}\en{ can be used},
\en{but}\ru{но}
\en{the~preference is given to}\ru{предпочтение отдаётся}
\en{the so~called}\ru{так называемым}
\inquotes{\en{inertial}\ru{инерциальным}}
\en{systems}\ru{системам},
\en{where}\ru{где}
\en{a~point}\ru{\hbox{точка}}
\en{moves}\ru{движется}
\en{with a~constant velocity}\ru{с~постоянной скоростью}
\en{without acceleration}\ru{без~ускорения}
(${\mathdotdotabove{\locationvector} = \hspace{-0.1ex} \zerovector}$, ${\mathdotabove{\locationvector} = \hspace{-0.1ex} \boldconstant}$)
\en{in the~absence}\ru{в~отсутствии}
\en{of external interactions}\ru{внешних взаимодействий}.

\en{A~motion}\ru{Движение}
\en{along a~straight line}\ru{вдоль прямой линии}
\en{with the~constant velocity}\ru{с~постоянной скоростью},
\en{also known as}\ru{также известное как}
\en{a~free motion}\ru{свободное движение},
\en{supposes}\ru{предполагает}
\en{the~absence}\ru{отсутствие}
\en{of external interactions}\ru{внешних взаимодействий}
(\en{or}\ru{или}
...
\en{Neither particle interacts with itself}\ru{Ни~одна частица не~взаимодействует сама с~собой}, ${\bm{F}^{\smthinternal}_{\hspace{-0.16ex}kk} \hspace{-0.25ex} = \hspace{-0.15ex} \zerovector \hspace{.3em} \forall k}$.
...
\bigl( \hspace{.1ex} \locationvector_k \hspace{-0.15ex} - \locationvector_{\hspace{-0.16ex}j} \hspace{.1ex} \bigr) \hspace{-0.35ex} \times \hspace{-0.2ex} \bm{F}^{\smthinternal}_{\hspace{-0.16ex}kj} = \zerovector
\hspace{.1ex} ,
\end{equation*}
...
\russianlanguage{Имея неподвижный базис~${\mathcircabove{\bm{e}}_i}$ \en{and}\ru{и} движущийся вместе с~телом базис~${\bm{e}_i}$, ...}

\russianlanguage{Если добавить базис~${\bm{e}_i}$ (этот базис движется вместе с~телом), то угловая ориентация тела может быть определена тензором поворота ${\rotationtensor \equiv \bm{e}_i \widetilde{\bm{e}}_i}$.}
...
\eqref{rodriguesrotationformula}, \chapterdotsectionref{chapter:mathapparatus}{section:rotationtensor}
...
\noindent
\en{where}\ru{где}~${\bm{F}_{k}^{\smthactive}}$\:---
...
${ \mathcolor{magenta}{\variation{\kineticenergydensity} = \scalebox{0.92}[0.95]{$ \displaystyle \sum_{\smash{k}} $} \hspace{.2ex} m_k \mathdotabove{\locationvector}_k \hspace{-0.16ex} \dotp \variation{\hspace{.1ex}\mathdotabove{\locationvector}_k}} \hspace{-0.5ex} }$ , \hspace{-0.1ex}
${%
\left(
...
= \hspace{-0.1ex} \scalebox{0.95}[0.96]{$ \displaystyle \integral\displaylimits_{\mathclap{t_1}}^{\raisemath{.12em}{\mathclap{t_2}}} $} \hspace{-0.1ex} \variation{\kineticenergydensity} \hspace{.1ex} dt \hspace{.16ex}
...
\AddUnderBrace[line width=.75pt][-0.1ex,-0.77em]{beginVariationOfKinetic}{endVariationOfKinetic}{${\scriptstyle \variation{\kineticenergydensity}}$}
%
...
\hspace{-0.8ex}
\left( \hspace{-0.25ex} \variation{\kineticenergydensity}
...
\vspace{-0.16em}\noindent
Без ущерба для общности можно принять ${\variation{\hspace{.1ex}\locationvector_k}\hspace{.1ex}(t_1) \hspace{-0.1ex} = \variation{\hspace{.1ex}\locationvector_k}\hspace{.1ex}(t_2) \hspace{-0.1ex} = \zerovector}$, тогда внеинтегральный член исчезает.
...
В~равновесии ${\locationvector_k \hspace{-0.12ex} = \boldconstant}$,
...
Пусть в~системе действуют два вида сил\::
потенциальные с~энергией от~обобщённых координат ${\potentialenergydensity (q^i)}$
и~дополнительные внешние~${\mathcircabove{Q}_{\hspace{-0.1ex}i}}$.
...
Известные теоремы статики линейно \textcolor{magenta}{упругих} систем легко доказываются в~случае конечного числа степеней свободы.
Теорема \href{https://en.wikipedia.org/wiki/Beno%C3%AEt_Paul_%C3%89mile_Clapeyron}{Clapeyron’а} выражается равенством
...
\AddOverBrace[line width=.75pt][-0.3ex,0.3em]{beginLegendreOfPotentialEnergy}{endLegendreOfPotentialEnergy}{${\scriptstyle d\potentialenergydensity}$}
...
До~этого не~ставился вопрос о~системе отсчёта, всё рассматривалось в~некой \inquotes{абсолютной} системе или одной из инерциальных систем~(\sectionref{section:initialconcepts.discreteapproach}).
...
\en{By}\ru{По}~\eqrefwithchapterdotsection{angularvelocityandbasisvectors}{chapter:mathapparatus}{section:rotationtensor}
