...
\en{even in the simplest cases}\ru{даже в~самых простых случаях},
...
\en{of the infinitesimal displacements}\ru{бесконечно малых перемещений}
\en{is applied everywhere}\ru{повсеместно применяется}.
\en{The equations}\ru{Уравнения}
\en{of this theory}\ru{этой теории}
...
\en{The~complete closed set}\ru{Полный замкнутый набор}~(\en{the system}\ru{система})
...
\nopagebreak\noindent
\en{Here}\ru{Здесь}
$\linearstress$\ru{\:---}\en{ is}
\en{the linear stress tensor}\ru{линейный тензор напряжения},
$\bm{f}$~\en{is the vector}\ru{--- вектор}
\en{of the volume loads}\ru{объёмных нагрузок},
$\infinitesimaldeformation$\ru{\:---}\en{ is}
\en{the infinitesimal linear deformation tensor}\ru{тензор бесконечномалой линейной деформации},
${
   \potential(
      \hspace{-0.1ex}\infinitesimaldeformation\hspace{-0.1ex}
   )\hspace{-0.1ex}
}$\ru{\:---}\en{~is}
\en{the potential energy of~deformation per volume unit}\ru{потенциальная энергия деформации единицы объёма},
${\stiffnesstensor}$\ru{\:---}\en{~is}
...
${
   \stiffnesstensor_{\hspace{.12ex} 12 \scalebox{.6}[.8]{$ \rightleftarrows $} 34}
   \hspace{-0.4ex} = \hspace{-0.1ex}
   \stiffnesstensor
}$, ${
   \stiffnesstensor_{\hspace{.12ex} 1 \scalebox{.6}[.8]{$ \rightleftarrows $} 2}
   \hspace{-0.33ex} = \hspace{-0.1ex}
   \stiffnesstensor
}$, ${
   \stiffnesstensor_{\hspace{.12ex} 3 \scalebox{.6}[.8]{$ \rightleftarrows $} 4}
   \hspace{-0.33ex} = \hspace{-0.1ex}
   \stiffnesstensor
}$
)%end brackets
...
\en{they can be derived via varying}\ru{они могут быть получены варьированием}
\en{of equations}
\en{of the~nonlinear theory}\ru{уравнений нелинейной теории}.
\en{The variation from an~arbitrary configuration}\ru{Варьирование от~произвольной конфигурации}
\en{is~described in}\ru{описано в}~\chapdotpararef{chapter:nonlinearcontinuum}{para:variationofconfiguration}.
\en{The~linear theory}\ru{Линейная теория}\ru{\:---}\en{ is}
...
= \scalebox{.9}{$ \displaystyle \frac{\raisemath{-0.125em}{\partial^2 \hspace{.1ex} \potential}}{\raisemath{-0.1em}{\partial \hspace{.1ex} \bm{C} \hspace{.1ex} \partial \hspace{.1ex} \bm{C}}} $}
...
\en{It remains to~change}\ru{Остаётся поменять}
    ${\variation{\currentlocationvector}}$
        ...
        $\bm{u}$,
    ${
        \infinimentpetitdeformationdevariation\hspace{-0.15ex}
    }$
\en{to}\ru{на}
    $\infinitesimaldeformation$,
   ...
   \hspace{-0.1ex}
    \raisemath{-0.3em}{\scalebox{.92}{$
        \partial \hspace{.1ex} \bm{C} \hspace{.1ex} \partial \hspace{.1ex} \bm{C}
    $}
    }
$}}$
\en{to}\ru{на}~$\stiffnesstensor$,
\en{and}\ru{а}~${ \hspace{-0.1ex} \rho \hspace{.25ex}
\variation{\hspace{-0.2ex}\bm{f}} }$
\en{to}\ru{на}
${ \hspace{-0.1ex} \bm{f} \hspace{-0.2ex} }$.

\en{If}\ru{Если} \en{the derivation}\ru{вывод}~\eqref{variationfrominitialconfiguration}
\en{seems abstruse to the~reader}\ru{кажется читателю малопонятным},
\en{it’s possible}\ru{возможно}
\en{to proceed}\ru{исходить}
\en{from}\ru{из}
\en{the next equations}\ru{следующих уравнений}
...
\en{the displacement}\ru{перемещение}~$\bm{u}$
...
\en{of the parameter}\ru{параметра}~$\smallparameter$
...
\en{In}\ru{В}~\hbox{\en{his book}\ru{его книге}~\cite{truesdell-firstcourse}}
\en{this is called}\ru{это названо}
\en{the }\inquotesx{\en{formal approximation}\ru{формальным приближением}}.
...
\en{after}\ru{после}
\en{the initial offset}\ru{начального смещения}
\en{of the amplitude of~vibrations}\ru{амплитуды вибраций},
\en{then}\ru{то}
...
\en{of the~initial dimensions}\ru{начальных размеров} }).
...
\ru{известны }\en{displacements}\ru{перемещения}\en{ are known},
\en{and}\ru{а}
\en{on}\ru{на}
...
\en{As was noted}\ru{Как отмечалось} \en{in}\ru{в}~\chapref{chapter:genericmechanics},
\en{the principle}\ru{принцип}
...
\en{also for}\ru{и~для}
\en{the linear theory}\ru{линейной теории}
...
\en{the virtual displacements}\ru{виртуальные перемещения}
