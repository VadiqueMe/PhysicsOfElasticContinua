...
\dropcap{С}{уществует} классическая, весьма развитая теория устойчивости движения~\cite{merkin-stabilityintro}.
По~Ляпунову, процесс устойчив, если м\'{а}лые начальные отклонения остаются м\'{а}лыми и~в~будущем.
\en{This as well applies to the~condition of~equilibrium.}\ru{Это также относится и~к~состоянию равновесия.}
Нужно рассмотреть динамику малых отклонений от равновесной конфигурации и~убедиться, что они не~растут.
...
\en{In~other words}\ru{Иными словами}, \inquotes{критическим} считается такое \en{equilibrium condition}\ru{равновесное состояние}, которое перестаёт быть изолированным,\:--- в~его окрестности появляется множество смежных равновесных форм.
...
\en{But there are more approaches.}\ru{Но есть ещё подходы.}
...
\en{Yet}\ru{Но} \en{that is pretty easy to~visualize}\ru{её довольно просто визуализировать} \en{for a~discrete model}\ru{для дискретной модели} (\en{with finite number of~degrees of~freedom}\ru{с~конечным числом степеней свободы}).
...
Ведь в~уравнения входит вариация~${\variation{\bm{q}}}$, она равна нулю лишь для \inquotes{\en{dead}\ru{мёртвых}} \en{loads}\ru{нагрузок}.
\textcolor{magenta}{Распространены} \inquotes{\en{tracking}\ru{следящие}} \en{loads}\ru{нагрузки}, то~есть определённым образом меняющиеся при смещениях частиц упругого тела.
