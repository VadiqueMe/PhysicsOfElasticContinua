...
\en{\section{Continuum. The two descriptions of processes}}
...
\en{are}\ru{суть}
\en{points}\ru{точки}
\en{with only}\ru{с~лишь}
\en{the translational degrees of~freedom}\ru{трансляционными степенями свободы}%
\footnote{\en{The translational degrees of~freedom}\ru{Трансляционные степени свободы}
\en{appear}\ru{возникают}
\en{from}\ru{из}
\en{the particle’s ability}\ru{способности частицы}
\en{to~move}\ru{двигаться}
\en{free}\ru{свободно}
\en{in the space}\ru{в~пространстве}.}\hbox{\hspace{-0.5ex}.}
\en{Thus}\ru{Поэтому}
\en{there’re no moments }\en{among the generalized forces}\ru{среди обобщённых сил\ru{ нет моментов}
\en{and no external couples}\ru{и~нет никаких внешних пар сил}\ru{,}
\en{applied}\ru{приложенных}
\en{to the~volume}\ru{к~объёму}
\en{or}\ru{или}
\en{to the~surface}\ru{к~поверхности}
...
\en{a~force}\ru{сила}~${\tractionvector{\currentunitnormal} \hspace{.2ex} d\mathcal{O}\hspace{-0.2ex}}$.
\textcolor{magenta}{%
...
\en{an~infinitely many}\ru{бесконечно много}
...
\en{It is}\ru{Это}
\en{a~more complex field}\ru{более сложное поле},
\en{an~infinite collection}\ru{бесконечная коллекция}
\en{of~all}\ru{всех}
\en{traction vectors}\ru{в\'{е}кторов тяги}
\en{for any}\ru{для любых}
\en{directions}\ru{направлений}
\en{of the infinitesimal surfaces}\ru{бесконечно-малых площ\'{а}док},
\en{containing}\ru{содержащих}
\en{this point}\ru{эту точку}
\en{in~self}\ru{в~себе}.

\en{Actually}\ru{На~с\'{а}мом деле},
\en{an~infinite set}\ru{бесконечное множество}~(\en{a~collection}\ru{коллекция})
...
\en{has}\ru{имеет}
три вещественных собственных числ\'{а}~$\mathsigma_i$, а~также тройку взаимно перпендикулярных собственных векторов единичной длины~(\chapdotpararef{chapter:mathapparatus}{para:eigenvectorseigenvalues}).
