...
\ru{\section{Контину\kern-0.11exум. Два описания процессов}}
...
\en{But anyway,}\ru{Но~всё~же} \en{it’s worth choosing}\ru{ст\'{о}ит выбрать} \en{a~}\en{fundamentally}\ru{фундаментально} \en{and}\ru{и}~\en{qualitatively}\ru{качественно} \en{different model}\ru{иную модель}\:--- \en{a~model of material continuum}\ru{модель сплошной среды~(материального контину\kern-0.11exума)}, \en{where mass}\ru{где масса} \en{is distributed}\ru{распределена} \en{in volume}\ru{в~объёме} \en{continuously}\ru{непрерывно}: \en{volume}\ru{объём}~$\mathcal{V}$ \en{contains}\ru{содержит} \en{mass}\ru{массу}

\nopagebreak\vspace{-0.2em}\ru{\vspace{-0.8em}}\begin{equation}
...
\vspace{-0.25em}\noindent
\en{here}\ru{здесь} ${\rho\hspace{.1ex}}$\ru{\:---}\en{ is} \en{volume(tric) mass density}\ru{объёмная плотность массы} \en{and}\ru{и}~${d \mathcal{V}}$\ru{\:---}\en{ is} \en{infinitesimal volume element}\ru{бесконечно\-малый элемент объёма}.

\en{Real matter}\ru{Реальная материя} \en{is modelled}\ru{моделируется} \en{as}\ru{как} \en{a~continuum}\ru{контину\kern-0.11exум}, \en{which can be thought of as}\ru{который может быть мыслим как} \en{an~infinite set}\ru{бесконечное множество} \en{of~vanishingly small}\ru{исчезающе м\'{а}лых} \en{particles}\ru{частиц}, \en{joined together}\ru{соединённых вместе}.
...
Возможны и~более сложные модели, в~которых частицы обладают степенями свободы не~только трансляции, но~и поворота, внутренней деформации и~другими.
Отметив, что подобные модели притягивают всё б\'{о}льший интерес, в~этой главе ограничимся классическим представлением о~среде как состоящей из~\inquotesx{простых точек}[.]

В~каждый момент времени~$t$ деформируемый контину\kern-0.11exум занимает некий объём~${\mathcal{V}\hspace{.1ex}}$ пространства.
Этот объём движется и~деформируется, но набор частиц в~нём постоянен\:--- \en{the~balance of~mass}\ru{баланс массы} (\inquotes{\en{matter is neither created nor annihilated}\ru{материя не~создаётся и~не~аннигилируется}})
...
\en{Introducing}\ru{Вводя} \en{some}\ru{какие\hbox{-}либо} \en{variable parameters}\ru{переменные параметры}~${q^{\hspace{.1ex}i}\hspace{-0.3ex}}$\:--- \en{curvilinear coordinates}\ru{криволинейные координаты}, \en{we have a~relation}\ru{имеем отношение} \en{for particle locations}\ru{для положений частиц}
...
\en{when}\ru{когда}
\en{instead of focusing on}\ru{вместо фокусирования на~том,} \en{how particles of~a~continuum}\ru{как частицы контину\kern-0.11exума} \en{move}\ru{движутся} \en{from the~initial configuration}\ru{из начальной конфигурации} \en{through space and~time}\ru{в~пространстве и~времени},
...
\en{and}\ru{и}~\en{aren’t confused}\ru{не~смущаемся} \en{by continuous}\ru{непрерывным} \en{leaving}\ru{уходом} \en{and}\ru{и}~\en{arriving}\ru{приходом} \ru{сюда }\en{of~various particles}\ru{разных частиц}\en{ here}.
...
Не~ст\'{о}ит противопоставлять материальное и~пространственное \en{descriptions}\ru{описания}.
В~этой книге используются оба в~зависимости от~ситуации.
...
= \bm{e}_i \hspace{.2ex} \scalebox{.9}{$ \displaystyle \frac{\raisemath{-0.2em}{\partial \initiallocationvector}}{\partial x_{i}} $} \hspace{-0.1ex}
...
Когда нет поворота~(${\rotationtensor = \hspace{-0.1ex} \UnitDyad \hspace{0.1ex}}$), тогда ${\bm{F} \hspace{-0.1ex} = \hspace{0.1ex} \bm{U} \hspace{-0.25ex} = \bm{V}\hspace{-0.3ex}}$.
...
\vspace{-0.2em}\noindent
\en{each of~which}\ru{каждый из~которых} \en{is sometimes called}\ru{иногда называется} \ru{тензором }\href{https://en.wikipedia.org/wiki/Gabrio_Piola}{Piola}\en{ tensor} \en{or}\ru{или} \ru{тензором }\href{https://en.wikipedia.org/wiki/Josef_Finger}{Finger}\ru{’а}\en{ tensor}.
\en{The~inverse}\ru{Обратный} \en{of left}\ru{к~левому} \ru{тензору }Cauchy--Green\ru{’а}\en{ tensor}~${\hspace{-0.1ex}\mathboldPhi\hspace{.1ex}}$\en{ is}\ru{\:---} \ru{тензор деформации }Cauchy\en{ deformation tensor}~${\hspace{-0.2ex}^2\hspace{-0.2ex}\bm{c}}$.
...
\bm{G} = \initiallocationvector^i \hspace{-0.25ex} \currentlocationvector_\differentialindex{i} \hspace{-0.1ex} \dotp \hspace{-0.15ex} \currentlocationvector_{\hspace{-0.1ex}j} \initiallocationvector^j \hspace{-0.25ex}
...
\currentlocationvector_\differentialindex{i} \hspace{-0.1ex} \dotp \hspace{-0.15ex} \currentlocationvector_\differentialindex{\hspace{-0.1ex}j}
\hspace{.1ex} ,
\\[.25em]
%
{^2\hspace{-0.4ex}\bm{f}} =  \initiallocationvector_\differentialindex{i} \currentlocationvector^{i} \hspace{-0.2ex} \dotp \hspace{-0.15ex} \currentlocationvector^j \hspace{-0.1ex} \initiallocationvector_\differentialindex{\hspace{-0.1ex}j} \hspace{-0.2ex}
...
= \textsl{g}_{i\hspace{-0.1ex}j} \currentlocationvector^{i} \hspace{-0.2ex} \currentlocationvector^{j}
\hspace{-0.3ex} , &
...
\\[.25em]
%
\mathboldPhi = \hspace{-0.1ex} \currentlocationvector_\differentialindex{i} \initiallocationvector^i \hspace{-0.15ex} \dotp \hspace{.1ex} \initiallocationvector^j \hspace{-0.25ex} \currentlocationvector_\differentialindex{\hspace{-0.1ex}j} \hspace{-0.2ex}
= \textsl{g}^{\hspace{.2ex}i\hspace{-0.1ex}j} \hspace{-0.2ex} \currentlocationvector_\differentialindex{i} \currentlocationvector_\differentialindex{\hspace{-0.1ex}j}
\hspace{.1ex} , &
\textsl{g}^{\hspace{.2ex}i\hspace{-0.1ex}j} \hspace{-0.15ex} \equiv
\initiallocationvector^i \hspace{-0.15ex} \dotp \hspace{.1ex} \initiallocationvector^j
\end{array}
\end{equation}
...
= \hspace{-0.2ex} \currentlocationvector_i \hspace{-0.1ex} \currentlocationvector^{i} \hspace{-0.2ex}
...
\vspace{-0.3em}\noindent
\en{but}\ru{но} ... lorem ipsum dolor sit amet where are the flowers gone wo sind sie geblieben
...
This tensor \textcolor{magenta}{gives the~\inquotes{square} of local change in distances} due to deformation: ${\displaystyle d\currentlocationvector \dotp d\currentlocationvector = d\initiallocationvector \dotp \bm{G} \dotp d\initiallocationvector}$
...
\nopagebreak\begin{equation*}
\displaystyle \bm{C} = \smalldisplaystyleonehalf ( \bm{G} - \UnitDyad \hspace{.1ex} )
\hspace{1em} \text{\en{or}\ru{или}} \hspace{1em}
...
{^2\hspace{-0.2ex}\bm{a}} = \smalldisplaystyleonehalf ( \UnitDyad - \hspace{-0.15ex} {^2\hspace{-0.2ex}\bm{c}} \hspace{.1ex} ) = \smalldisplaystyleonehalf (\UnitDyad - \mathboldPhi^{\expminusone})
\hspace{1em} \text{\en{or}\ru{или}} \hspace{1em}
...
\en{So}\ru{Итак}, \en{there’s}\ru{есть} \en{velocity field}\ru{поле скоростей} \en{in spatial description}\ru{в~пространственном описании} ${\bm{v} \equiv \hspace{-0.1ex} \mathdotabove{\currentlocationvector} = \bm{v}(\currentlocationvector, \hspace{-0.1ex} t)}$.
\en{Decomposition}\ru{Разложение} \en{of~tensor}\ru{тензора} ${\hspace{-0.1ex}\boldnabla \bm{v} \hspace{-0.16ex} = \hspace{-0.2ex} \boldnabla \hspace{-0.16ex} \mathdotabove{\currentlocationvector} = \hspace{-0.16ex} \currentlocationvector^{i} \partial_i \mathdotabove{\currentlocationvector} = \hspace{-0.16ex} \currentlocationvector^{i} \hspace{-0.2ex} \mathdotabove{\currentlocationvector}_i}$%
...
\frac{\raisemath{-0.2em}{\partial}}{\partial q^i} \frac{\raisemath{-0.2em}{\partial \hspace{-0.1ex} \currentlocationvector}}{\partial t}
= \frac{\raisemath{-0.2em}{\partial}}{\partial t} \frac{\raisemath{-0.2em}{\partial \hspace{-0.1ex} \currentlocationvector}}{\partial q^i}
$}
\hspace{.8em} \text{\en{or}\ru{или}} \hspace{.8em}
\partial_i \mathdotabove{\currentlocationvector} = \hspace{-0.1ex} \mathdotabove{\currentlocationvector}_i
\end{equation*}
}
...
\nopagebreak\vspace{-0.1em}\begin{equation*}
\boldnabla \hspace{-0.16ex} \mathdotabove{\currentlocationvector}
= \hspace{-0.2ex} \boldnabla \hspace{-0.16ex} \mathdotabove{\currentlocationvector}^{\mathsf{S}} \hspace{-0.25ex}
- \hspace{.1ex} \smalldisplaystyleonehalf \hspace{-0.33ex} \left( \hspace{-0.2ex} \boldnabla \hspace{-0.15ex}\times\hspace{-0.25ex} \mathdotabove{\currentlocationvector} \right) \hspace{-0.4ex} \times \hspace{-0.2ex} \UnitDyad
\end{equation*}

\vspace{-0.1em}\noindent
\en{or}\ru{или}, \en{introducing}\ru{вводя} \en{rate of~deformation tensor}\ru{тензор скорости деформации}~(\ru{rate of~deformation tensor, }rate of~stretching tensor, strain rate tensor)~${\rateofdeformationtensor}$ \en{and}\ru{и}~\en{vorticity tensor}\ru{тензор вихря}~(\ru{vorticity tensor, }rate of~rotation tensor, spin tensor)~${\vorticitytensor}$
...
=  \hspace{-0.2ex} \boldnabla \hspace{-0.16ex} \mathdotabove{\currentlocationvector}^{\mathsf{S}} \hspace{-0.32ex}
= \smalldisplaystyleonehalf \hspace{-0.33ex} \left( \hspace{-0.3ex} \currentlocationvector^{i} \hspace{-0.2ex} \mathdotabove{\currentlocationvector}_i \hspace{-0.1ex} + \hspace{-0.1ex} \mathdotabove{\currentlocationvector}_i \currentlocationvector^{i} \right)
\hspace{-0.44ex} ,
\\[.44em]
%
...
= \hspace{.1ex} \smalldisplaystyleonehalf \hspace{.15ex} \currentlocationvector^{i} \hspace{-0.4ex}\times\hspace{-0.35ex} \mathdotabove{\currentlocationvector}_i
\hspace{.1ex} ,
\end{array}
\end{equation}
...
\ru{Компоненты}\en{Components} \en{of the~rate of~deformation tensor}\ru{тензора скорости деформации} \en{in current configuration’s basis}\ru{в~базисе текущей конфигурации}
...
= \hspace{-0.1ex} \currentlocationvector_\differentialindex{i} \hspace{-0.1ex} \dotp \rateofdeformationtensor \hspace{-0.15ex} \dotp \hspace{-0.15ex} \currentlocationvector_{\hspace{-0.1ex}j} \hspace{-0.1ex}
= \hspace{.1ex} \smalldisplaystyleonehalf \hspace{.25ex} \currentlocationvector_\differentialindex{i} \hspace{-0.1ex} \dotp \hspace{-0.25ex} \left( \hspace{-0.25ex} \currentlocationvector^{k} \hspace{-0.2ex} \mathdotabove{\currentlocationvector}_{k} \hspace{-0.1ex} + \hspace{-0.1ex} \mathdotabove{\currentlocationvector}_k \currentlocationvector^{k} \hspace{-0.1ex} \right) \hspace{-0.4ex} \dotp \hspace{-0.15ex} \currentlocationvector_\differentialindex{\hspace{-0.1ex}j} \hspace{-0.2ex}
=
\\[-0.3em]
\shoveright{ \hfill \hspace{12em}
= \hspace{.1ex} \smalldisplaystyleonehalf \hspace{-0.33ex} \left( \hspace{-0.25ex} \mathdotabove{\currentlocationvector}_\differentialindex{i} \hspace{-0.1ex} \dotp \hspace{-0.15ex} \currentlocationvector_\differentialindex{\hspace{-0.1ex}j} \hspace{-0.1ex} + \hspace{-0.1ex} \currentlocationvector_\differentialindex{i} \hspace{-0.1ex} \dotp \hspace{-0.15ex} \mathdotabove{\currentlocationvector}_\differentialindex{\hspace{-0.1ex}j} \hspace{-0.1ex} \right) \hspace{-0.3ex}
...
\en{called}\ru{называемому} \en{Nanson’s formula}\ru{формулой Nanson’а}.

\en{\section{Forces in continuum. Cauchy stress tensor}}

\ru{\section{Силы в контину\kern-0.11exуме. Тензор напряжения Cauchy}}
...
\en{Force}\ru{Сила}~${\rho \bm{f} d\mathcal{V}}$ действует на бесконечно-малый объём~$d\mathcal{V}$; \hbox{если}~$\bm{f}$\:--- массовая сила (действующая на~единицу массы), то~${\rho \bm{f}}$\:--- объёмная.
Такие силы происходят от силовых полей, например: гравитационные силы (\inquotes{силы тяжести}), силы инерции в~неинерциальных системах отсчёта, электромагнитные силы при~наличии в~среде зарядов и~токов.

\en{Surface force}\ru{Поверхностная сила}~${\bm{p} \hspace{.2ex} d\mathcal{O}\hspace{-0.12ex}}$ действует на бесконечно-малую поверхность~${d\mathcal{O}\hspace{-0.12ex}}$.
Это может~быть \en{contact pressure}\ru{контактное давление} \en{or/and}\ru{или/и}~\en{friction}\ru{трение}, электростатическая сила при~сосредоточенных на~поверхности зарядах.
...
\en{Internal forces}\ru{Внутренние силы} \en{balance the~action}\ru{уравновешивают действие} \en{of~external forces}\ru{внешних сил}, \en{and}\ru{и}~\en{inside the~medium}\ru{внутри среды} \en{they are transmitted}\ru{они передаются} \en{continuously}\ru{непрерывно} \en{from~point to~point}\ru{от~точки к~точке}.
\en{Since times}\ru{Со~времён} \en{of~}Euler\ru{’а} \en{and}\ru{и}~Cauchy, \en{internal forces}\ru{внутренние силы} \en{in a~continuum}\ru{в~контину\kern-0.11exуме} \en{are assumed to be}\ru{считаются} \en{surface short-range contact forces}\ru{поверхностными контактными силами близкодействия}:
\en{on infinitesimal surface}\ru{на бесконечномалой площ\'{а}дке}~${\hspace{-0.1ex}\currentunitnormal d\mathcal{O}\hspace{-0.12ex}}$ \en{inside a~medium}\ru{внутри среды} \en{acts}\ru{действует} \en{force}\ru{сила}~${\tractionvector{\currentunitnormal} \hspace{.2ex} d\mathcal{O}\hspace{-0.2ex}}$.
\textcolor{magenta}{\en{Clarifying}\ru{Проясняя}(??): \en{acts}\ru{действует} \en{from?? the~side}\ru{с~той?? стороны}\ru{,} \en{where}\ru{куда} \ru{направлена }\en{unit normal}\ru{единичная нормаль}~$\currentunitnormal$\en{ is directed}.}

\en{By the~action\hbox{--}reaction principle}\ru{По принципу действия и~противодействия}, $\tractionvector{\currentunitnormal}$ меняет знак с~переменой направления~$\currentunitnormal$ на~противоположную сторону:
${\tractionvector{-\currentunitnormal} = - \hspace{.2ex} \tractionvector{\currentunitnormal}}$.
(\en{Sometimes}\ru{Иногда} \en{the~last}\ru{последний} \en{thesis}\ru{тезис} \en{is proved}\ru{доказывается} \en{via}\ru{через} \en{the~balance of~momentum}\ru{баланс импульса} \en{for an~infinitely short cylinder}\ru{для бесконечно короткого цилиндра} \en{with bases}\ru{с~основаниями} ${\currentunitnormal \hspace{.1ex} d\mathcal{O}\hspace{-0.1ex}}$ \en{and}\ru{и}~${- \hspace{.1ex} \currentunitnormal \hspace{.1ex} d\mathcal{O}\hspace{-0.2ex}}$.)

\en{Vector}\ru{Вектор}~${\tractionvector{\currentunitnormal}}$ \en{on the~surface}\ru{на~площ\'{а}дке} \en{with unit normal}\ru{с~единичной нормалью}~$\currentunitnormal$ \en{is called}\ru{называется} \en{the~}\ru{вектором поверхностной тракции~(}surface traction vector\ru{)} \en{or}\ru{или} \en{force\hbox{-}stress vector}\ru{вектором силового напряжения}.
\en{However}\ru{Однако}, ${\tractionvector{\currentunitnormal}}$\en{ is}\ru{\:---} \en{not a~vector field}\ru{не векторное поле}: \en{traction}\ru{тракция} ${\tractionvectoritself \hspace{.1ex} \narroweq \hspace{.2ex} \tractionvectoritself(\currentunitnormal, \locationvector, t)}$ \en{depends}\ru{зависит} \en{not only on location}\ru{не~только от положения}~$\locationvector$ \en{of~the~point}\ru{точки}, \en{but also}\ru{но также} \en{on the~local orientation}\ru{от локальной ориентации} (\en{defined by}\ru{определяемой}~$\currentunitnormal$) \en{of the~surface element}\ru{элемента поверхности}.
\en{An~infinite number of~surfaces}\ru{Бесконечное число площ\'{а}док} \en{of any orientation}\ru{любой ориентации} \en{pass}\ru{проходят} \en{through}\ru{через} \en{the~same point}\ru{одну и~ту~же точку}, \en{and}\ru{и}~\en{infinitely many}\ru{бесконечно много} \en{vectors}\ru{векторов}~$\tractionvector{\currentunitnormal}$ \en{exist}\ru{существуют} \en{at~each point}\ru{в~каждой точке}.

\textcolor{magenta}{\en{Thereby}\ru{Тем самым}, \en{stress at the~point of~a~continuum}\ru{напряжение в~точке контину\kern-0.11exума}\en{ is not a~vector field}\ru{\:--- не~векторное поле}, \en{but}\ru{но} совокупность~(множество) \en{of~all traction vectors}\ru{всех векторов тракции} для любых по\hbox{-}всякому ориентированных площ\'{а}док, содержащих в~себе эту точку.}
\en{And it turns out}\ru{И~оказывается}, \en{the~infinite set}\ru{бесконечное множество} \en{of~all vectors}\ru{всех векторов}~$\tractionvector{\currentunitnormal}$ \en{at the~point}\ru{в~точке} \en{is completely determined}\ru{полностью определяется} \en{by the~only one second complexity tensor}\ru{одним-единственным тензором второй сложности}\:--- \ru{тензором напряжения }Cauchy\en{ stress tensor}~$\cauchystress$.
Рассмотрим \textcolor{magenta}{содержащийся во~многих книгах} вывод этого утверждения\:--- \en{the~theorem}\ru{теорему} \en{on existence}\ru{о~существовании} \en{of the }\en{now-called}\ru{ныне называемого} \ru{тензора напряжения }Cauchy\en{ stress tensor}\:--- \en{and not without}\ru{и~не~без} \inquotesx{\en{the~tetrahedron argument}\ru{аргумента-тетраэдра}}[.]

{\small
A. L. Cauchy founded Continuum Mechanics by assuming that two parts of a continuum interact by means of surface densities of contact forces concentrated on their dividing surface.
Then, assuming that these contact forces depend only on the normal to the dividing surface and that they are balanced by some volume density of force (including inertia), he played with tetrahedrons and proved the existence of the stress tensor.
\par}
...
% \href{https://en.wikipedia.org/wiki/Augustin-Louis_Cauchy}{\bibauthor{Augustin\hbox{-}Louis Cauchy}}. \emph{Exercices de math\'{e}matiques.} Troisi\`{e}me ann\'{e}e: 1828. Paris, Chez de~Bure fr\`{e}res.
...
ограниченный поверхностью~${\mathcal{O}(\boundary \mathcal{V})}$,
нагруженный поверхностными~${\bm{p} \hspace{.2ex} d\mathcal{O}}$ и~массовыми/объёмными~${\bm{f}dm \hspace{-0.15ex} = \hspace{-0.1ex} \rho \bm{f} d\mathcal{V}}$ внешними силами.
Формулировка баланса импульса такова

\nopagebreak\vspace{-0.2em}\begin{equation}\label{balanceoftranslationalmomentum.integral}
\displaystyle\left( \integral\displaylimits_{\mathcal{V}} \hspace{-0.32ex} \rho \hspace{.2ex} \bm{v} \hspace{.1ex} d\mathcal{V} \hspace{-0.22ex} \right)^{\hspace{-0.32em}\tikz[baseline=-0.2ex]\draw[black, fill=black] (0,0) circle (.28ex);} \hspace{-0.1ex}
=
...
\noindent
Импульс слева найдём по~\eqref{rateofvolumeintegralchange}, а~поверхностный интеграл справа превратим в~объёмный по~теореме о~дивергенции. Получим
...
\vspace{-0.25em}\noindent
Но объём~$\mathcal{V}$ произволен, поэтому равно нулю подынтегральное выражение.
...
Теперь к~балансу момента импульса.
Интегральная формулировка:

\nopagebreak\vspace{-0.32em}\begin{equation}\label{balanceofrotationalmomentum.integral}
\displaystyle\left( \integral\displaylimits_{\mathcal{V}} \hspace{-0.32ex} \currentlocationvector \times \hspace{-0.2ex} \rho \hspace{.2ex} \bm{v} \hspace{0.1ex} d\mathcal{V} \hspace{-0.25ex} \right)^{\hspace{-0.32em}\tikz[baseline=-0.2ex]\draw[black, fill=black] (0,0) circle (.28ex);} \hspace{-0.1ex}
...
\nopagebreak\vspace{-0.2em}\begin{equation*}
\displaystyle\left( \integral\displaylimits_{\mathcal{V}} \hspace{-0.4ex} \currentlocationvector \times \hspace{-0.2ex} \rho \hspace{.1ex} \mathdotabove{\currentlocationvector} \hspace{.4ex} d\mathcal{V} \hspace{-0.2ex} \right)^{\hspace{-0.32em}\tikz[baseline=-0.2ex]\draw[black, fill=black] (0,0) circle (.28ex);} \hspace{-0.1ex}
=
...
\hspace{.3ex} +
\integral\displaylimits_{\mathcal{V}} \hspace{-0.32ex} \tikzmark{RdotCrossRdotBegin} \mathdotabove{\currentlocationvector} \times \hspace{-0.2ex} \rho \hspace{.1ex} \mathdotabove{\currentlocationvector} \hspace{.1ex} \tikzmark{RdotCrossRdotEnd} \hspace{.3ex} d\mathcal{V} ,
\end{equation*}
...
= - \hspace{-0.4ex} \integral\displaylimits_{\mathcal{V}} \hspace{-0.5ex} \boldnabla \dotp \left( \cauchystress \times \hspace{-0.24ex} \currentlocationvector \hspace{.2ex} \right) \hspace{-0.1ex} d\mathcal{V} ,
\end{multline*}
...
\nopagebreak\vspace{-0.2em}\begin{equation*}
\tikzmark{divergenceOfStressCrossLocationBegin} \hspace{-0.2ex} \boldnabla \dotp \hspace{.1ex} \left( \cauchystress \times \hspace{-0.24ex} \currentlocationvector \hspace{.24ex} \right) \hspace{-0.33ex} \tikzmark{divergenceOfStressCrossLocationEnd}
= \hspace{-0.33ex} \tikzmark{firstTermOfStressCrossLocationDivergenceBegin} \hspace{.1ex} \left( \hspace{.12ex} \boldnabla \dotp \cauchystress \hspace{.2ex} \right) \hspace{-0.25ex} \times \hspace{-0.2ex} \currentlocationvector \hspace{.2ex} \tikzmark{firstTermOfStressCrossLocationDivergenceEnd} \hspace{.15ex}
...
\currentlocationvector^{i} \hspace{-0.25ex} \dotp \hspace{-0.1ex} \left( \cauchystress \hspace{-0.2ex} \times \hspace{-0.2ex} \partial_i \currentlocationvector \hspace{.24ex} \right) \hspace{-0.1ex}
= \currentlocationvector^{i} \hspace{-0.2ex} \dotp \hspace{-0.1ex} \left( \hspace{.1ex} \bm{e}_{j} \tractionvector{j} \hspace{-0.2ex} \times \hspace{-0.2ex} \currentlocationvector_{\hspace{.1ex}i} \hspace{.12ex} \right) \hspace{-0.16ex}
...
\eqrefwithchapdotpara{gradientofcrossproductoftwovectors}{chapter:elementsoftensorcalculus}{para:spatialdifferentiationoftensorfields}
\,\Rightarrow &
...
(${J \hspace{-0.1ex} \equiv \operatorname{det} \bm{F}\hspace{-0.12ex}}$\en{ is}\ru{\:---} \en{Jacobian}\ru{якобиан}, \en{determinant of the motion gradient}\ru{определитель градиента движения}),
...
= \hspace{-0.2ex} \boldnablacircled \currentlocationvector^{\T} \hspace{-0.2ex}}$\ru{\:---}\en{ is} \en{motion gradient}\ru{градиент движения}, ${J \hspace{-0.1ex} \equiv \operatorname{det} \bm{F}\hspace{-0.12ex}}$\en{ is}\ru{\:---} \en{Jacobian}\ru{якобиан} (\en{Jacobian determinant}\ru{определитель Якоби}).}
...
\nopagebreak\vspace{-0.2em}\begin{equation}
\variation{\potential} = \hspace{.1ex} \firstpiolakirchhoffstress \hspace{-0.12ex} \dotdotp \hspace{.1ex} \variation{\hspace{.1ex} \boldnablacircled \currentlocationvector^{\T}}
...
\vspace{-0.2em} \noindent --- этот немного неожиданный результат получился благодаря коммутативности $\variation$ и~$\smash{\hspace{-0.1ex}\boldnablacircled\hspace{.1ex}}$: ${\boldnablacircled \hspace{.1ex} \variation{\currentlocationvector}^{\hspace{.2ex}\T} \hspace{-0.4ex} = \variation{\hspace{.1ex} \boldnablacircled \currentlocationvector^{\T}}\hspace{-0.25ex}}$ ($\boldnabla$ \en{and}\ru{и}~$\variation$ \en{don’t commute}\ru{не~коммутируют}).

% энергетически сопряжённый с ... = energy conjugate to ...

Тензор~$\firstpiolakirchhoffstress$ оказался энергетически сопряжённым с~${\bm{F} \equiv \hspace{-0.1ex} \smash{\boldnablacircled} \hspace{-0.16ex} \currentlocationvector^{\T}}$

\nopagebreak\vspace{-0.12em}\begin{equation}
\firstpiolakirchhoffstress \hspace{-0.1ex}
= \scalebox{0.92}{$ \displaystyle \frac{\partial \hspace{.1ex} \potential}{\raisemath{-0.4em}{\partial \hspace{.1ex} \smash{\boldnablacircled} \hspace{-0.16ex} \currentlocationvector^{\T}}} $}
...
= \firstpiolakirchhoffstress \hspace{-0.1ex} \dotdotp \hspace{-0.2ex} \boldnablacircled \hspace{.1ex} \variation{\currentlocationvector}^{\hspace{0.12ex}\T}
\\[.25em]
%
\variation{\potential}
= \hspace{-0.16ex} \left(^{\mathstrut} \hspace{-0.1ex} \mathcircabove{\rho} \bm{f} + \hspace{-0.25ex} \boldnablacircled \dotp \firstpiolakirchhoffstress \right) \hspace{-0.4ex} \dotp \variation{\currentlocationvector}
\hspace{.1ex}
+ \firstpiolakirchhoffstress \hspace{-0.1ex} \dotdotp \hspace{-0.2ex} \boldnablacircled \hspace{.1ex} \variation{\currentlocationvector}^{\hspace{0.12ex}\T}
\end{array}
\end{equation*}
...
The~first one is non-symmetric, it connects forces in the~deformed stressed configuration to underfomed geometry+mass (initially known volumes, areas, densities), and it is energetically conjugate to the motion gradient (commonly mistakenly called \inquotes{deformation gradient}, despite comprising of rigid rotations). The~first (or sometimes its transpose) is also known as \inquotes{nominal stress} and \inquotes{engineering stress}.

The~second one is symmetric, it connects loads in the~initial undeformed configuration to initial mass+geometry, and it’s conjugate to the right Cauchy\hbox{--}Green deformation tensor (and thus to the Cauchy\hbox{--}Green\hbox{--}Venant measure of deformation).
...
There’s also popular Cauchy stress, which relates forces in the~deformed configuration to deformed geometry+mass.
...
{\small
In the case of finite deformations, the Piola\hbox{--}Kirchhoff stress tensors express the stress relative to the initial configuration. This is in contrast to the Cauchy stress tensor which expresses the stress relative to the present configuration. For infinitesimal deformations and rotations, the Cauchy and Piola\hbox{--}Kirchhoff tensors are identical.

Whereas the Cauchy stress tensor~${\cauchystress}$ relates stresses in the current configuration, the motion gradient and strain tensors are described by relating the motion to the initial configuration; thus not all tensors describing the material are in either the initial or current configuration. Describing the stress, strain and deformation either in the initial or current configuration would make it easier to define constitutive models. For example, the Cauchy stress tensor is variant to a pure rotation, while the deformation strain tensor is invariant; thus creating problems in defining a constitutive model that relates a varying tensor, in terms of an invariant one during pure rotation; as by definition constitutive models have to be invariant to pure rotations.

\subsection*{1st Piola\hbox{--}Kirchhoff stress tensor}

The \emph{1st~Piola\hbox{--}Kirchhoff stress tensor} is one possible solution to this problem. It defines a family of tensors, which describe the configuration of the body in either the current or the initial configuration.

The 1st Piola\hbox{--}Kirchhoff stress tensor~$\bm{T}$ relates forces in the present~(\inquotes{spatial}) configuration with areas in the initial~(\inquotes{material}) configuration
\[
...
Whereas the 1st~Piola\hbox{--}Kirchhoff stress relates forces in the current configuration to areas in the initial configuration, the 2nd~Piola\hbox{--}Kirchhoff stress tensor~$\bm{S}$ relates forces in the initial configuration to areas in the initial configuration. The force in the initial configuration is obtained via a mapping that preserves the relative relationship between the force direction and the area normal in the initial configuration.
\[
...
%% ${\widetilde{\currentlocationvector} \equiv \hspace{-0.1ex} \variation{\currentlocationvector}}$
...
Прежде упругая среда рассматривалась в~двух конфигурациях: начальной~(отсчётной) с~векторами\hbox{-}радиусами~$\initiallocationvector$ и~текущей~(актуальной) с~$\currentlocationvector$.
\hbox{Теперь} представим~себе малое изменение текущей конфигурации с~бесконечно\-малыми приращениями
вектора положения~$\variation{\currentlocationvector}$,
...
= \hspace{-0.2ex} \boldnabla \dotp \hspace{-0.16ex} \boldnablacircled \initiallocationvector \hspace{-0.08ex}
= \currentlocationvector^{i} \hspace{-0.1ex} \partial_i \hspace{-0.16ex} \dotp \initiallocationvector^j \hspace{-0.1ex} \partial_{\hspace{-0.08ex}j} \initiallocationvector \hspace{-0.08ex}
\stackrel{?}{=} \currentlocationvector^{i} \hspace{-0.1ex} \partial_i \initiallocationvector \hspace{-0.1ex} \dotp \initiallocationvector^j \hspace{-0.1ex} \partial_{\hspace{-0.08ex}j} \hspace{-0.32ex}
= \hspace{-0.2ex} \boldnabla \initiallocationvector \dotp \hspace{-0.16ex} \boldnablacircled \hspace{-0.1ex}
...
= \initiallocationvector^i \partial_i \hspace{-0.16ex} \dotp \hspace{-0.12ex} \currentlocationvector^{j} \hspace{-0.1ex} \partial_{\hspace{-0.08ex}j} \hspace{-0.1ex} \currentlocationvector
\stackrel{?}{=} \initiallocationvector^i \partial_i \currentlocationvector \dotp \hspace{-0.24ex} \currentlocationvector^{j} \hspace{-0.1ex} \partial_{\hspace{-0.08ex}j} \hspace{-0.32ex}
= \hspace{-0.32ex} \boldnablacircled \currentlocationvector \hspace{.1ex} \dotp \hspace{-0.16ex} \boldnabla \hspace{-0.25ex}
...
= \hspace{-0.2ex} \left( \hspace{.1ex} \scalebox{0.92}{$ \displaystyle \frac{\partial^2 \hspace{.1ex} \potential}{\raisemath{-0.1em}{\partial \hspace{.1ex} \bm{C} \hspace{.1ex} \partial \hspace{.1ex} \bm{C}}} $} \dotdotp \variation{\hspace{.12ex}\bm{C}} \hspace{-0.16ex} \right) \hspace{-0.32ex} \dotp \bm{F}^{\hspace{.1ex}\T} \hspace{-0.24ex}
+ \hspace{.1ex}
...
= \hspace{-0.12ex} \bm{F}^{\hspace{.1ex}\T} \hspace{-0.25ex} \dotp \boldnabla \hspace{.1ex} \variation{\currentlocationvector} \hspace{.16ex},
\:\,
\variation{\bm{F}} \hspace{-0.2ex} = \variation{\hspace{.1ex} \boldnablacircled \currentlocationvector^{\T}} \hspace{-0.4ex}
= \hspace{-0.2ex} \boldnabla \hspace{.1ex} \variation{\currentlocationvector}^{\hspace{.12ex}\T} \hspace{-0.25ex} \dotp \bm{F}
\hspace{-0.1ex} ,
\\[.5em]
%
\variation{\hspace{.12ex}\bm{C}} = \bm{F}^{\hspace{.1ex}\T} \hspace{-0.32ex} \dotp \hspace{.1ex} \infinimentpetitdeformationdevariation \dotp \bm{F} ,
\:\,
...
\:\Rightarrow\:
%
\initialunitnormal \hspace{.1ex} do = J^{\hspace{.12ex}\expminusone} \currentunitnormal \hspace{.1ex} d\mathcal{O} \hspace{-0.1ex} \dotp \bm{F}
\:\Rightarrow\:
%
