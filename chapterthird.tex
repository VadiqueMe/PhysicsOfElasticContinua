...
\lettrine[lines=2, findent=2pt, nindent=0pt]{\en{A}\ru{С}}{\en{ccording}\ru{огласно}} \en{to~}\href{https://en.wikipedia.org/wiki/Atomic_theory}{\en{the~atomic theory}\ru{атомной теории}}, \en{a~substance}\ru{вещество} \en{is composed}\ru{состоит} \en{of discrete particles}\ru{из дискретных частиц}\:--- \en{atoms}\ru{атомов}.
\en{Therefore}\ru{Поэтому} \en{a~model}\ru{модель} \en{of a~system of~particles}\ru{системы частиц} \en{with }\ru{с~}\en{masses}\ru{массами}~${m_k}$ \en{and}\ru{и}~\en{position vectors}\ru{радиусами-векторами}~${\bm{R}_k (t)}$ \en{may seem suitable}\ru{может показаться подходящей} \en{yet despite}\ru{даже несмотр\'{я} на} \en{an~unimaginable}\ru{невообразимое} \en{number of degrees of freedom}\ru{число степеней свободы}, \en{because}\ru{так как} \en{amounts of~memory}\ru{объёмы памяти} \en{and}\ru{и}~\en{speed}\ru{быстрота} \en{of~computers}\ru{компьютеров} \en{are characterized}\ru{характеризуются} \en{also}\ru{тоже} \en{by }\en{astronomical numbers}\ru{астрономическими числами}.
...
m = \hspace{-0.5ex} \scalebox{0.9}{$\displaystyle\integral\displaylimits_{\mathcal{V}}$} \hspace{-0.2ex} \rho \hspace{.2ex} d\mathcal{V}
, \hspace{.5em}
dm = \rho \hspace{.15ex} d \mathcal{V} ,
\end{equation}

\vspace{-0.25em}\noindent \en{here}\ru{здесь} ${\rho\hspace{.1ex}}$\ru{\:---}\en{ is} \en{volume(tric) mass density}\ru{объёмная плотность массы}.
...
\vspace{-0.1em} \noindent \en{and randomness of}\ru{и~случайность}~${\mathcal{V}\hspace{-0.2ex}}$, \en{we get}\ru{получаем} \en{the local differential version of}\ru{локальную дифференциальную версию}~\eqref{princlipleofvirtualwork.integral:nonlinearmomentlesscontinuum}
