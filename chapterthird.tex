...
\en{Convenient}\ru{Удобная} \en{relation}\ru{связь} \en{to}\ru{с}~$\bm{F}$\:---  \en{without}\ru{без} \en{extracting roots}\ru{извлечения корней}, \ru{в~отличие от}\en{unlike for}~${\bm{U}\hspace{-0.25ex}}$ \en{and}\ru{и}~${\bm{V}\hspace{-0.25ex}}$\:--- \en{explains}\ru{объясняет} \en{the~big popularity}\ru{больш\'{у}ю популярность} \en{of~tensors}\ru{тензоров}~$\bm{G}$ \en{and}\ru{и}~$\mathboldPhi$.
...
\en{So}\ru{Итак}, \en{there’s}\ru{есть} \en{velocity field}\ru{поле скоростей} \en{in spatial description}\ru{в~пространственном описании} ${\bm{v} \equiv \mathdotabove{\bm{R}} = \bm{v}(\bm{R}, t)}$.
...
= \hspace{-0.2ex} \boldnabla \hspace{-0.16ex} \mathdotabove{\bm{R}}^{\hspace{.25ex}\mathsf{S}} \hspace{-0.25ex}
...
=  \hspace{-0.2ex} \boldnabla \hspace{-0.16ex} \mathdotabove{\bm{R}}^{\hspace{.25ex}\mathsf{S}} \hspace{-0.32ex}
...
\en{Since}\ru{Поскольку} \en{particles of this model of~continuum}\ru{частицы этой модели континуума}\en{ are}\ru{\:---} \en{points with only translational degrees of~freedom}\ru{точки лишь с~трансляционными степенями свободы}, %%(translational degrees of freedom arise from a~particle’s ability to move freely in space)
\ru{то }\en{there’re no moments among generalized forces}\ru{среди обобщённых сил нет моментов}.
...
\footnote{\en{As before}\ru{По\hbox{-}прежнему} ${\bm{F}\hspace{-0.12ex}}$\:--- градиент движения \en{and}\ru{и}~${J \hspace{-0.1ex} \equiv \operatorname{det} \bm{F}\hspace{-0.12ex}}$\:--- \en{Jacobian}\ru{якобиан}.}
%% (определитель Якоби, Jacobian determinant)
