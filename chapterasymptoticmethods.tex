...
\en{all}\ru{все}
\en{these parameters}\ru{эти параметры}
\en{were thought}\ru{мыслились}
\en{to be known}\ru{известными}.
%
\en{All}\ru{Все}
\en{asymptotic methods}\ru{асимптотические методы}
\en{are based}\ru{основаны}
\en{on the~study of}\ru{на изучении того}\ru{,}
\en{how the~solution}\ru{как решение}
\en{depends on the~parameters}\ru{зависит от параметров}.

\en{A~decomposition}\ru{Разложение}
\en{of~perturbation}\ru{возмущения}
\en{usually consists}\ru{обычно состоит}
\en{of the first two terms}\ru{из первых двух слагаемых}.
%
\en{When}\ru{Когда}
\en{a~problem}\ru{задача}
\en{with unknown}\ru{с~неизвестной}~$u$
\en{contains parameter}\ru{содержит параметр}~$a$\ru{,}
\en{then}\ru{то}, \en{assuming}\ru{полагая}
...
\en{is sought}\ru{ищется}
\en{as a~series}\ru{в~виде ряда}
...
\vspace{-0.2em}\noindent
\en{Additional argument}\ru{Дополнительный аргумент}~$\smallparameter$
\en{is called}\ru{называется}
\en{a~formal small parameter}\ru{формальным малым параметром}.

\en{Although}\ru{Хоть}
\en{a~decomposition}\ru{разложение}
\en{of~perturbation}\ru{возмущения}
\en{may diverge}\ru{и~может расходиться},
\en{it}\ru{оно}
\en{may}\ru{может}\ru{,}
\en{nevertheless}\ru{тем не~менее}\ru{,}
\en{describe}\ru{описывать}
\en{the~solution}\ru{решение}
\en{more useful}\ru{полезнее}\ru{,}
\en{than}\ru{чем}
\en{representations}\ru{представления}
\en{with }\ru{с~}\en{the~uniform}\ru{равномерной}
\en{and }\ru{и~}\en{absolute}\ru{абсолютной}
\en{convergence}\ru{сходимостью}.
...
\en{of the~perturbation methods}\ru{методов возмущений}
...
\phi = \scalebox{.8}{$ \displaystyle\sum_{k=0}^{\infty} $} \hspace{.3ex}
\phi_{k}(\smallparameter)
\hspace{.1ex} ,
\hspace{.5em}
\underset{\scriptstyle \smallparameter \to 0}{\operatorname{lim}} \hspace{.25ex}
\displaystyle\frac{\phi - \scalebox{.8}{$ \displaystyle\sum_{k=0}^{n} $} \hspace{.1ex}
\phi_{k}}{\phi_{n}} = 0
\hspace{.1ex} .
\end{equation}
...
\en{the~remainder}\ru{остаток}
\en{of~series}\ru{ряда}
\en{is}\ru{есть}
\en{smaller}\ru{меньше}
---\:\en{of a~higher degree}\ru{более высокой степени}
\en{of~infinitesimality}\ru{бесконечно-малости}\:---
\en{than}\ru{чем}
\en{the~last}\ru{последний}
\en{withheld}\ru{удержанный}
\en{term}\ru{член}.
%
\en{Obviously}\ru{Очевидно},
\en{a~power series}\ru{степенной ряд}~\eqref{asymptoticsolution}
\en{also converges as}\ru{сходится и~как}
\en{an~asymptotic series}\ru{асимптотический ряд}.

\en{But}\ru{Но}
\en{the decomposable unknowns}\ru{разлагаемые неизвестные}
\en{usually}\ru{обычно}
\en{also}\ru{также}
\en{depend on}\ru{зависят от}
\en{the~}\inquotes{\en{main}\ru{главных}}
\en{arguments}\ru{аргументов}\:---
\en{the~coordinates}\ru{координат}
\en{and }\ru{и~}\en{time}\ru{времени}.
%
\en{The~convergence}\ru{Сходимость}
\en{for}\ru{для}~${\smallparameter \to 0}$
\en{must be}\ru{должна быть}
\en{uniform}\ru{равномерной}
\en{across these}\ru{по этим}
\inquotes{\en{main}\ru{главным}}
\en{arguments}\ru{аргументам}\:---
\en{this is a~requirement}\ru{это требование}
\en{for the~effective use}\ru{к~эффективному использованию}
\en{of asymptotic decompositions}\ru{асимптотических разложений}.
%
...
\vspace{-0.2em}\noindent
\en{doesn’t satisfy}\ru{не удовлетворяет}
\en{the~requirement}\ru{требованию}
\en{of~uniformity}\ru{равномерности},
\en{because}\ru{поскольку}
\en{the~subsequent terms}\ru{последующие члены}
\en{prevail}\ru{преобладают}
\en{over}\ru{над}
\en{the~previous ones}\ru{предыдущими}
\en{when}\ru{когда}
${t \to \infty}$.

\en{Why}\ru{Чем~же}
\en{asymptotic methods}\ru{асимптотические методы}
\en{are so attractive}\ru{так привлекательны}?
%
\en{As example}\ru{Как пример}
\en{take a~solution}\ru{возьмём решение}
\en{of the~equation}\ru{уравнения}
...
\en{decomposition}\ru{разложение}~\eqref{asymptoticsolution}
\en{into}\ru{в}~\eqref{powerseries-asymptotic}
\en{and }\ru{и~}\en{equating}\ru{приравнивая}
\en{coefficients}\ru{коэффициенты}
\en{before}\ru{перед}
\en{the~same degrees}\ru{одинаковыми степенями}\en{ of}~$\smallparameter$,
\en{we get}\ru{получим}
...
\en{But another way is possible too}\ru{Но возможен и~другой путь}\::
\en{if it’s known that}\ru{если известно, что}
\en{some parameter}\ru{некий параметр}~${\omega}$
\en{influences the~solution only a~little}\ru{лишь немного влияет на решение},
\en{then}\ru{то},
\en{redesignating it}\ru{переобозначив его}
\en{as}\ru{как}~${\smallparameter \omega}$,
\textcolor{magenta}{\en{do asymptotic analysis}\ru{сделать асимптотический анализ}}
\en{for}\ru{для}~${\smallparameter \to 0}$.

\en{Surely}\ru{Разумеется},
\en{these are not \en{the\;}\inquotes{laws} of~asymptotics}\ru{это не \inquotes{законы} асимптотики},
\en{but only considerations}\ru{а~лишь соображения}.
\en{There’s no common theory}\ru{Нет общей теории}
\en{of~asymptotic methods}\ru{асимптотических методов},
\en{and}\ru{и}
\en{their application}\ru{их применение}
\en{is to some extent}\ru{это в~какой-то степени}
\en{an art}\ru{искусство}.

\en{The~deeper}\ru{Более глубокое}
\en{description}\ru{описание}
\en{of asymptotic methods}\ru{асимптотических методов}
\en{is written}\ru{написано}
\en{in the books}\ru{в~книгах}
\en{by }Ali Hasan\ru{’а} Nayfeh~\cite{nayfeh-introtoperturbation, nayfeh-perturbation}.

\en{\section{Splitting in a linear algebraic system}}

\ru{\section{Расщепление в линейной алгебраической системе}}

\en{This simple case}\ru{Этот простой случай}
\en{well}\ru{отлично}
\en{illustrates}\ru{иллюстрирует}
\en{asymptotic methods}\ru{асимптотические методы}.
...
C_{i\hspace{-0.1ex}j} \hspace{-0.2ex} = C_{i\hspace{-0.1ex}j}^{\hspace{0.2ex}\scalebox{.66}{(0)}} \hspace{-0.2ex} + \smallparameter \hspace{.2ex} C_{i\hspace{-0.1ex}j}^{\hspace{0.2ex}\scalebox{.66}{(1)}}
\end{equation}

\noindent
\en{with }\ru{с~}\en{the~matrix}\ru{матрицей}~$C_{i\hspace{-0.1ex}j}$
\en{and }\ru{и~}\en{the~sets}\ru{наборами}
\en{of~unknowns}\ru{неизвестных}~$u_j$
\en{and }\ru{и~}\en{loads}\ru{нагрузок}~$f_i$.
\en{The~process}\ru{Процесс}
\en{of~constructing}\ru{построения}
\en{an~asymptotic solution}\ru{асимптотического решения}
\en{is determined by}\ru{определяется тем}\ru{,}
\en{whether}\ru{сингулярна (обратима)} \en{the matrix}\ru{матрица}~${C_{i\hspace{-0.1ex}j}^{\hspace{.2ex}\scalebox{.66}{(0)}}}$\en{ is singular (invertible)}
...
\indent
\textboldextended{1$^{\hspace{-0.1ex}\circ}$\hspace{-1ex}.}\, ${\determinant \hspace{.16ex} C_{i\hspace{-0.1ex}j}^{\hspace{0.2ex}\scalebox{.66}{(0)}} \hspace{-0.25ex} \neq 0}$.
Однородная задача
...
\indent \textboldextended{2$^{\hspace{.1ex}\circ}$\hspace{-1.1ex}.}\, ${\determinant \hspace{.16ex} C_{i\hspace{-0.1ex}j}^{\hspace{0.2ex}\scalebox{.66}{(0)}} \hspace{-0.25ex} = 0}$

...


\indent \textboldextended{3$^{\hspace{.1ex}\circ}$\hspace{-1.1ex}.}\, ${\determinant \hspace{.16ex} C_{i\hspace{-0.1ex}j}^{\hspace{0.2ex}\scalebox{.66}{(0)}} \hspace{-0.25ex} = 0}$
