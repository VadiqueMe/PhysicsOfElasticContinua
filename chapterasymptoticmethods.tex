...
\en{were only coordinates and time}\ru{были только координаты и~время}.
\en{Dimensions of a~body}\ru{Размеры тела},
\en{elastic moduli}\ru{упругие модули},
\en{ranges of action changes}\ru{диапазоны изменений воздействия}
...
\en{perturbation methods}\ru{методы возмущений}
\en{use}\ru{используют}
\en{another approach}\ru{иной подход}:
\en{series}\ru{ряды}
\en{are considered}\ru{рассматриваются}
\en{as asymptotic}\ru{как асимптотические}
\en{with the~convergence}\ru{со сходимостью}
${\smallparameter \to 0}$,
\en{not}\ru{не}~${n \to \infty}$
(n~\textcolor{magenta}{\en{shows}\ru{показывает}}
\en{how many terms to hold}\ru{как много членов удерживать}).
...
\en{by the~value of~load}\ru{по величине нагрузки}~(\chapterdotsectionref{chapter:linearclassicalelasticity}{para:wholesetofequations.lineartheory}).
...
\en{of the~heat equation}\ru{уравнения теплопроводности}~\eqrefwithchapterdotsection{heatequation:mathematicalphysics}{chapter:thermoelasticity}{para:heatequation}
