...
\en{skew-symmetric}\ru{кососимметричный},
\en{so}\ru{так что}
...
\en{Linear}\ru{Линейная} \en{and}\ru{и}~\en{angular}\ru{угловая}~\eqrefwithchapterdotsection{angularvelocityvector}{chapter:mathapparatus}{section:rotationtensor} \en{velocities}\ru{скорости} \en{of a~rod’s particle}\ru{частицы стержня} \en{are}\ru{суть}
...
\en{in a~one-dimensional}\ru{в~одномерной}
\en{model}\ru{модели}.
Если взять,
\en{for example}\ru{например},
\en{such a~version}\ru{такую версию}
(\en{the index}\ru{индекс}
\en{of}\ru{у}~$\bm{u}$
\en{shows}\ru{показывает}
\en{the dimensions of the model}\ru{размерность модели})
...
При этом, чтобы уйти от~различий в~трактовках ${\bm{u}_{\raisemath{-0.2ex}{1}}}$ и~$\bm{\theta}$, будем исходить из \en{complementary energy}\ru{дополнительной энергии}~${\widehat{\potentialenergydensity}(\internalmoment, \internalforce)}$:
