...
\dropcap{С}{тержень}\:--- это тонкое длинное тело.
Он характеризуется прежде всего своей осью\:--- пространственной кривой, которую \inquotes{облепляет} материал (?? \textcolor{blue}{рисунок} ??).
В~каждой точке оси имеем плоскую фигуру

...

\en{In approximate applied theories of~rods}\ru{В~приближённых прикладных теориях стержней} (\inquotes{\en{strength of~materials}\ru{сопротивление материалов}}) фигурируют внутренние силы~$\bm{Q}$ и~моменты~$\mathboldM$.
С~тензором напряжения они связаны соотношениями
...
Наиболее логичный подход к~описанию деформации упругих стержней связан с~асимптотическим расщеплением трёхмерной задачи при малой толщине.
Однако для сложной асимптотической процедуры заранее нужен некий вариант ответа.
Такой вариант даёт прямой подход, основанный на одномерной модели стержня как материальной линии.
Но какими степенями свободы\:--- кроме трансляции\:--- должны обладать частицы этой линии?

Давно известно, что стержни чувствительны к~моментным нагрузкам.
...
Во\hbox{-}первых, это моментные модели, и~моменты здесь играют главную роль~(не~роль поправок, как в~трёхмерном контину\kern-0.11exуме Коссера).
Во\hbox{-}вторых, стержни являются как~бы \inquotes{тестовой площадкой} для~моделей с~дополнительными степенями свободы, поскольку наличие этих степеней можно достоверно исследовать на~трёх\-мерной модели.

Ну~а~пока сосредоточимся на~простой моментной одномерной модели типа Cosserat.
...
Вместо тройки материальных координат ${q^{\hspace{.1ex}i}}$ имеем одну\:--- $s$, это может быть дуговая координата в~начальной конфигурации.
Движение определяется вектором положения~${\locationvector(s,t)}$ \en{and}\ru{и}~тензором поворота~${\rotationtensor(s,t)}$.
...
Внутренними взаимо\-действиями будут сила~${\mathboldQ \hspace{.1ex} (s)}$ и~момент~${\mathboldM \hspace{.1ex} (s)}$\:--- это воздействие от~частицы с~координатой~${s\!+\!0}$ к~частице с~${s\!-\!0}$.
\en{The~action--re\-action principle}\ru{Принцип действия--про\-ти\-во\-действия} даёт, что перемена направления~$s$ меняет знаки ${\mathboldQ}$ и~${\mathboldM}$.
...
В~классической теории Кирхгофа существует внутренняя связь
...
Рассматривается прямой стержень, защемлённый на одном конце и~нагруженный силой~$\mathboldQ$ на~другом (рисунок ?? 123 ??).
Сила \inquotes{мёртвая}~(не~меняется при~деформировании)
...
\en{Kirchhoff’s model}\ru{Модель Kirchhoff’а} \en{with internal constraint}\ru{с~внутренней связью}~${\bm{\Gamma} \hspace{-0.25ex}=\hspace{-0.2ex} \bm{0}}$ \en{doesn’t describe the~simplest case of extension/compression for a~straight rod}\ru{не~описывает простейшего случая растяжения/сжатия для прямого стержня}.
...
\en{adding the~possibility of (ex)tension}\ru{добавлением возможности растяжения} \en{and}\ru{и} \en{inhibiting only transverse shear}\ru{подавлением лишь поперечного сдвига}, \en{id~est}\ru{то~есть}
...
При м\'{а}лой относительной толщине стержня модель типа Cosserat уступает место классической.
Понятие \inquotes{толщина} определяется соотношением жёсткостей: $\bm{a}$, $\bm{b}$ и~$\bm{c}$\:--- разной размерности; полагая ${\bm{a} = h^{\hspace{-0.12ex}2} \hspace{.2ex} \widearc{\bm{a}}}$ и~${\bm{c} = h \hspace{.16ex} \widearc{\bm{c}}}$, где~$h$\:--- некий масштаб длины, получим тензоры

...


Переход модели Cosserat в~классическую кажется более очевидным при непосредственном интегрировании
...
Трудно переоценить ту роль, которую играет в~механике стержней классическое решение Saint\hbox{-\hspace{-0.2ex}}Venant’а.
...
\bm{u}_{\raisemath{-0.2ex}{1}}\hspace{-0.15ex}(z) \hspace{-0.1ex} = \hspace{.1ex} \surfaceexpminusone \hspace{-0.4ex} \integral\displaylimits_{\surface} \hspace{-0.5ex} \bm{u}_{\raisemath{-0.2ex}{3}}\hspace{-0.05ex}(\bm{x},z) \hspace{.25ex} d\surface
\hspace{.15ex} ,
\:\;
\bm{\theta}(z) \hspace{-0.1ex} = \hspace{.1ex} \smalldisplaystyleonehalf \hspace{.4ex} \surfaceexpminusone \hspace{-0.4ex} \integral\displaylimits_{\surface} \hspace{-0.5ex} \boldnabla \hspace{-0.15ex} \times \hspace{-0.15ex} \bm{u}_{\raisemath{-0.2ex}{3}} \hspace{.2ex} d\surface
\hspace{.15ex} ,
\end{equation*}
...
Помимо $\mathboldQ$ и~$\mathboldM$, есть ещё величина, не вызывающая сомнений\:--- упругая энергия.
Естественно потребовать, чтобы в~одно\-мерной и в~трёх\-мерной моделях энергии на~единицу длины совпали.
При этом, чтобы уйти от~различий в~трактовках ${\bm{u}_{\raisemath{-0.2ex}{1}}}$ и~$\bm{\theta}$, будем исходить из \en{complementary energy}\ru{дополнительной энергии}~${\widehat{\potential}(\mathboldM, \mathboldQ)}$:

\nopagebreak\vspace{-0.1em}\begin{equation*}
\widehat{\potential}(\mathboldM, \mathboldQ) = \hspace{-0.2ex} \integral\displaylimits_{\surface} \hspace{-0.4ex} \potential_{\raisemath{-0.2ex}{3}} \hspace{.2ex} d\surface
\end{equation*}
...
Для вариационного построения одномерных моделей удобен принцип Рейсснера\hbox{--}Хеллингера~(\chapdotpararef{chapter:linearclassicalelasticity}{para:mixedvariationalprinciples}) с~независимой аппроксимацией напряжений~\cite{eliseev-models}. Но тогда необходима некая согласованность между $\bm{u}$ и~$\cauchystress$.
...
Одномерные модели составляют лишь часть картины; другая часть\:--- это двумерные задачи в~сечении, а~вместе они являются тем решением трёхмерной задачи, которое образуется при~м\'{а}лой толщине.
...
Прямой подход, столь эффективный при~построении одномерных моделей Cosserat и~Kirchhoff’а, для~проблем термоупругости непримен\'{и}м.

Переход от трёхмерной модели может быть реализован или~вариационным путём, или~асимптотическим.
...
Для этого нужно~(\chapdotpararef{chapter:thermoelasticity}{para:variationalformulations.thermoelasticity}) заменить в~принципе Lagrange’а
...
В~отличие от~других тем теории упругости, стержни в~книгах представлены весьма скромно.
Преобладает изложение в~стиле сопротивления материалов, более точные и~совершенные подходы большинству авторов кажутся невозможными или~ненужными.
