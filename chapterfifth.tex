...
\en{But}\ru{Но}
\en{it is not so hard}\ru{не~так~уж трудно}
\en{to build the more complex models}\ru{построить более сложные модели}
\en{of a~continuous medium}\ru{сплошной среды}, \en{where}\ru{где}
\en{the particles}\ru{частицы}
\en{have}\ru{обладают}
\en{not only}\ru{не~только~лишь}
\en{the degrees of the translation freedom}\ru{степенями свободы трансляции},
\en{but also}\ru{но~и}
\en{the some additional ones}\ru{н\'{е}которыми дополнительными}.
\en{These new degrees of freedom}\ru{Эти новые степени свободы}
\en{are related}\ru{относятся}
\en{to the new loads}\ru{с~новыми нагрузками}.
...
\en{by the }\ru{братьями }Cosserat\en{ brothers} \en{in}\ru{в}~1909~\cite{cosserat}.
...
\en{the interest to this topic arose}\ru{возник интерес к~этой теме}.
...
\en{on the virtual displacements}\ru{на виртуальных смещениях}
...
\en{In}\ru{В}~\chapterdotsectionref{chapter:nonlinearcontinuum}{section:stressesAsLagrangeMultipliers} \en{for the~momentless continuum}\ru{для безмоментной среды}, \en{stresses appear as Lagrange multipliers}\ru{напряжения появляются как множители Lagrange’а} \en{in the~principle of~virtual work}\ru{в~принципе виртуальной работы}, \textcolor{magenta}{\en{when}\ru{когда}}~${\mathcolor{magenta}{\variation{\internalwork} \hspace{-0.2ex} = 0}}$.
...
\vspace{-0.25em}\noindent
После интегрирования с~применением теоремы о~дивергенции\footnote{${\bm{a} \dotp (\somebivalenttensor \dotp \bm{c} \hspace{.1ex}) = (\bm{a} \dotp \hspace{-0.12ex} \somebivalenttensor) \hspace{-0.1ex} \dotp \bm{c} = \bm{a} \dotp \hspace{-0.12ex} \somebivalenttensor \dotp \bm{c}}$}
...
Если устремить $\elcursive$ к~нулю, то исчезает вклад $\wrynesstensor$ в~$\potentialenergydensity$, а~с~ним и моментные напряжения~$\couplestresstensor$.
...
\en{of the~deformation ten\-sors}\ru{тензоров деформации}~\eqref{deformationtensors:micropolarcontinuum}
...
для конечного числа
степеней свободы
(минимальность энергии,
\en{the}\ru{теорема} Clapeyron’\en{s}\ru{а}\en{ theorem},
\en{the reciprocal work theorem}\ru{теорема о~взаимности работ}
\en{et~al.}\ru{и~др.}\:--- \chapterdotsectionref{chapter:genericmechanics}{section:statics}),
справедливые
\en{for}\ru{для}
\en{any}\ru{любой}
\en{continual}\ru{континуальной}
\en{linear}\ru{линейной}
\en{elastic}\ru{упругой}
\en{medium}\ru{среды}~(\chapterref{chapter:linearclassicalelasticity}),
\en{including}\ru{включая}
\en{the~micropolar model of~continuum}\ru{микрополярную модель \rucontinuum{}а}
(\en{a~medium with the~force couples}\ru{среду с~парами сил}
\en{or}\ru{или}
\en{the~moments}\ru{моментами}).
...
\en{as}\ru{как}
\en{for}\ru{для}
\en{the~classical momentless continuum}\ru{классического безмоментного \rucontinuum{}а}:
...
Равенство~${\distortiontensor_{\hspace{-0.25ex}\Xcompanion} \hspace{-0.32ex} = \bm{0}}$~(симметрию~$\distortiontensor$) возможно понимать как внутреннюю связь~(\chapterdotsectionref{chapter:nonlinearcontinuum}{section:internalconstraints}).
...
В~классической (линейной безмоментной) теории упругости полная система сводится к~одному уравнению для~вектора~$\fieldofdisplacements$~(\chapterdotsectionref{chapter:linearclassicalelasticity}{section:equationsfordisplacements.linearelasticity}).
В~моментной теории
