...
\en{In}\ru{В}~\chapterdotsectionref{chapter:nonlinearcontinuum}{section:stressesAsLagrangeMultipliers}
\en{stresses}\ru{напряжения}
\en{appear}\ru{появляются}
\en{as}\ru{как}
\ru{множители }Lagrange\ru{’а}\en{ multipliers}
...
\en{For an~isotropic medium}\ru{Для изотропной среды} ${\mathboldoe = \zerovector}$, ${\inertiatensor = \inertiascalar \UnitDyad}$.
...
Равенство~${\distortiontensor_{\hspace{-0.25ex}\Xcompanion} \hspace{-0.32ex} = \zerovector}$~(симметрию~$\distortiontensor$) возможно понимать как внутреннюю связь~(\chapterdotsectionref{chapter:nonlinearcontinuum}{section:internalconstraints}).
