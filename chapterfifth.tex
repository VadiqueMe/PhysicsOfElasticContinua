...
\vspace{-0.1em}
\en{Introducing the strain (the deformation) tensors}\ru{Вводя тензоры деформации}\:---
...
\eqrefwithchapterdotpara{pseudovectorinvariant}{chapter:mathapparatus}{para:tensors.symmetric+skewsymmetric}
\:\Rightarrow\,
...
\en{et~al.}\ru{и~др.}\:--- \chapterdotsectionref{chapter:genericmechanics}{para:statics}),
справедливые
\en{for}\ru{для}
\en{the continual}\ru{континуальных}
\en{linear elastic media}\ru{линейных упругих сред}~(\chapterref{chapter:linearclassicalelasticity}),
\en{including}\ru{включая}
\en{the~micropolar model of~continuum}\ru{микрополярную модель \rucontinuum{}а}~(\en{a~medium with the force couples}\ru{среду с~парами сил}
\en{or}\ru{или}
...
\en{like}\ru{как}
\en{in the~classical momentless continuum}\ru{в~классическом безмоментном \rucontinuum{}е}:
...
Равенство~${\distortiontensor_{\hspace{-0.25ex}\Xcompanion} \hspace{-0.32ex} = \bm{0}}$~(симметрию~$\distortiontensor$) возможно понимать как внутреннюю связь~(\chapterdotsectionref{chapter:nonlinearcontinuum}{para:internalconstraints}).
...
В~классической (линейной безмоментной) теории упругости полная система сводится к~одному уравнению для~вектора~$\fieldofdisplacements$~(\chapterdotsectionref{chapter:linearclassicalelasticity}{para:equationsfordisplacements.linearelasticity}).
В~моментной теории
...
\textcolor{magenta}{После тех~же преобразований, что и~в~\sectionref{para:introtolinearmicropolar}, получаем}
...
Вспомним переход к~модели со~стеснённым вращением в~линейной теории~(\sectionref{para:caseoflatenttrihedron.smalldisplacementsandrotations}).
