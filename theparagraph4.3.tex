...
\en{the tensor}\ru{тензор}~$\stiffnesstensor$
\en{is symmetric}\ru{симметричен}
\ru{ещё~и~}\en{also inside}\ru{внутри}
...
\en{of the independent components}\ru{независимых компонент}
\en{to}\ru{до}~21:
...
\en{is characterized}\ru{характеризуется}
\en{by the set of orthogonal tensors}\ru{набором ортогональных тензоров}~\bm{Q}.
\en{The set must solve the equation}\ru{Набор должен решать уравнение}
${ \bm{Q} \dotp \bm{Q}^\T = \UnitDyad }$
...
\begin{equation}\label{Hooke's law for the symmetric crystals}
   ...
   \Bigl( \bm{Q} \dotp \lineardeformation \dotp \bm{Q}^\T \Bigr)
   = \bm{Q} \dotp
...
\en{is characterized}\ru{характеризуется}
\en{by the set of orthogonal tensors}\ru{набором ортогональных тензоров}~$\bm{Q}$\ru{,}
...
which depends only on invariants from the solution of the characteristic equation

\en{a~function}\ru{функция}\ru{,}
\en{its arguments are only invariants}\ru{её аргументы суть только инварианты}
en{}\ru{из решения характеристического уравнения}~\eqref{......}.
...
function with members of the second degree, but not higher


\en{For an~isotropic material}\ru{Для изотропного материала}\en{,}
 \eqref{the Hooke's law for the symmetric crystals} \en{is satisfied}\ru{удовлетворяется}
\en{by any orthogonal tensor}\ru{любым ортогональным тензором}~$\bm{Q}$.
\en{It’s easier to accept that}\ru{Проще принять, что}~${ \potential(\lineardeformation) }$
...
\en{from the solution of the characteristic equation}\ru{из решения характеристического уравнения}~\eqref{}
