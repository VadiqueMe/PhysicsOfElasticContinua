...
\en{in each orthonormal basis}\ru{в~каждом ортонормальном базисе}~${\bm{e}_i}$
\en{we have a~set}\ru{имеем набор}
\en{of~nine}\ru{девяти}~(${3^2 \hspace{-0.24ex}=\hspace{-0.12ex} 9}$)
\en{numbers}\ru{чисел}
$\somebivalenttensorcomponents{i\hspace{-0.1ex}j}$~(${i, j = 1, 2, 3}$),
\en{and this set is transformed}\ru{и этот набор преобразуется}
\en{during a~transition}\ru{во~время перехода}
\en{to~a~new~(rotated) orthonormal basis}\ru{к~новому~(повёрнутому) ортонормальному базису}~${\bm{e}'_i}$
\en{as}\ru{как}
...
\en{a~tensor}\ru{тензор}~$\somebivalenttensor$
\en{of the~second complexity}\ru{второй сложности}
(\en{of the~second valence}\ru{второй валентности},
\en{bivalent}\ru{бивалентный}).

\en{In other words}\ru{Иными словами},
\en{tensor}\ru{тензор}~$\somebivalenttensor$
\en{reveals in every basis}\ru{проявляется в~каждом базисе}
\en{as a~collection of its components}\ru{как совокупность своих компонент}~$\somebivalenttensorcomponents{i\hspace{-0.1ex}j}$,
\en{changing along with a~basis}\ru{меняющейся вместе с~базисом}
\en{according to}\ru{согласно}~\eqref{orthotransform:2}.

% dyad

\en{The~key example}\ru{Ключевой пример}
\en{of a~second complexity tensor}\ru{тензора второй сложности}\en{ is}\ru{\:---}
\en{a~dyad}\ru{диада}.
\en{Having two vectors}\ru{Имея два вектора}
${\bm{a} \hspace{-0.2ex} = \hspace{-0.2ex} a_i \bm{e}_i}$
\en{and}\ru{и}~${\bm{b} \hspace{-0.2ex} = \hspace{-0.2ex} b_i \bm{e}_i}$,
\en{in each basis}\ru{в~каждом базисе}~${\bm{e}_i}$
\en{assume}\ru{пол\'{о}жим}
${d_{i\hspace{-0.1ex}j} \hspace{-0.2ex} \equiv a_i b_j}$.
\en{It’s easy to~see}\ru{Легко увидеть}
\en{how components}\ru{как компоненты}~${d_{i\hspace{-0.1ex}j}}$
\en{transform}\ru{преобразуются}
\en{according to}\ru{согласно}~\eqref{orthotransform:2}:
...
\vspace{-0.25em}\noindent
\en{A~resulting}\ru{Получающийся}
\en{tensor}\ru{тензор}~${^2\hspace{-0.25ex}\bm{d}}$
\en{is called}\ru{называется}
\ru{диадным произведением~(}\en{a~}dyadic product\ru{)}
\en{or}\ru{или}
\en{just}\ru{просто}
\ru{диадой~(}dyad\ru{)}
\en{and }\ru{и~}\en{is written as}\ru{пишется как}~${\bm{a} \hspace{-0.15ex} \otimes \hspace{-0.15ex} \bm{b}}$
\en{or}\ru{или}~${\bm{a} \bm{b}}$.
...
\en{When}\ru{Когда}
\en{some bivalent tensor}\ru{какой\hbox{-}то бивалентный тензор}~$\somebivalenttensor$\en{ is}\ru{\:---}
\en{a~dyad}\ru{диада}~${\bm{\beta}\bm{b}}$,
\en{its components}\ru{его компоненты}
${ \somebivalenttensorcomponents{i\hspace{-0.1ex}j} \hspace{-0.3ex} = \hspace{-0.2ex} \beta_{i} b_{\hspace{-0.1ex}j} }$
\en{satisfy}\ru{удовлетворяют}
\en{the~equality}\ru{равенству}
${ \somebivalenttensorcomponents{pq} \somebivalenttensorcomponents{mn} \hspace{-0.2ex} = \somebivalenttensorcomponents{mq} \somebivalenttensorcomponents{pn} }$\ru{,}
\en{to get}\ru{чтобы получить}
\en{commutativity}\ru{коммутативность}
\en{of~multiplication}\ru{умножения}
${ \beta_{p} b_{q} \hspace{.2ex} \beta_{m} b_{n} \hspace{-0.2ex} = \beta_{m} b_{q} \hspace{.2ex} \beta_{p} b_{n} }$.
\en{Here}\ru{Здесь}~${p \hspace{-0.2ex} \neq \hspace{-0.2ex} m}$,
\en{or else}\ru{а~ин\'{а}че}
\en{the~equality}\ru{равенство}
\en{becomes}\ru{стан\'{о}вится}
\en{the~identity}\ru{т\'{о}ждеством}.

% unit dyad

\en{The essential}\ru{Существенный}
\en{bivalent tensor}\ru{бивалентный тензор}
\ru{является}\en{is}
\en{the~unit tensor}\ru{единичный тензор}~(\en{other names}\ru{другие именования}\ru{\:---}\en{ are}
\en{unit dyad}\ru{единичная диада},
\en{identity tensor}\ru{тождественный тензор}
\en{and }\ru{и~}\en{metric tensor}\ru{метрический тензор}).
\en{Let}\ru{Пусть}
\en{for any}\ru{для~любого}
\en{orthonormal}\ru{отронормального}~(\ru{декартова, }cartesian)
\en{basis}\ru{базиса}
...
\en{These are really components of~tensor}\ru{Это действительно компоненты тензора},
\eqref{orthotransform:2}
\en{is actual}\ru{актуально}:
...
\en{I~write}\ru{Я~пишу}
\en{this tensor}\ru{этот тензор}
\en{as}\ru{как}~$\UnitDyad$
(\en{other}\ru{другие}
\en{popular}\ru{популярные}
\en{choices}\ru{альтернативы}\ru{\:---}\en{ are}
$\bm{I}$
\en{and}\ru{и}~${\hspace{-0.1ex} ^2\hspace{-0.1ex}\bm{1}}$).
...
\en{of the~null}\ru{нулевого},
\en{or}\ru{или}
\inquotesx{\en{vanishing}\ru{исчезающего}}[,]
\en{vector}\ru{вектора}~$\zerovector$
\en{are equal to zero}\ru{равны нулю}
\en{within any basis}\ru{в~любом базисе}).
...
\en{et~al.}\ru{и~др.}
%
\en{But what are}\ru{Но что такое}
\en{components}\ru{компоненты},
\en{for example}\ru{к~примеру},
\en{of~a~vector}\ru{вектора}
${\bm{v} = v_i \bm{e}_i}$, ${v_i = \bm{v} \dotp \bm{e}_i}$?
\en{If}\ru{Если}
\en{not scalars}\ru{не~скаляры},
\en{then}\ru{то} \en{what}\ru{что}?
%
\en{Here could be no simple answer}\ru{Здесь не~может быть простого ответа}.
%
\en{In~each}\ru{В~каждом}
\en{particular basis}\ru{отдельном базисе},
${\bm{e}_i}$\ru{\:---}\en{ are}
\en{vectors}\ru{векторы}
\en{and}\ru{и}~${v_i}$\ru{\:---}\en{ are}
\en{scalars}\ru{скаляры}.
