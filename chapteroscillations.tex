...
\vspace{.2em}
\en{According to the common theory}\ru{Согласно общей теории}~(\chapdotpararef{chapter:genericmechanics}{para:smalloscillations}), \en{we begin}\ru{мы начинаем} \en{with the analysis}\ru{с~анализа} \en{of~harmonics}\ru{гармоник}~(\en{ortho\-gonal oscillations}\ru{орто\-гональ\-ных колебаний}):
...
\ru{Тождество }Clapeyron’\en{s}\ru{а}\en{ identity}~\eqrefwithchapdotpara{clapeyron:elasticitytheorem}{chapter:linearclassicalelasticity}{para:theoremsofstatics} \en{gives}\ru{даёт}
...
\vspace{-0.2em}\noindent
\en{It also means that}\ru{Это также значит, что} ${\omega^2 \hspace{-0.2ex} \geq 0}$, \en{with zero}\ru{с~нулём} \en{for}\ru{для} \en{motion}\ru{движения}~$\mathboldU$ \en{of~a~continuum}\ru{контину\kern-0.11exума} \en{as a~rigid whole}\ru{как жёсткого целого}.
\en{If}\ru{Если} \en{at~least}\ru{хотя~бы} \en{a~small part}\ru{малая часть} \en{of the surface}\ru{поверхности} \en{is fixed}\ru{закреплена}, \en{then}\ru{то} \en{all}\ru{все}~${\omega_i \hspace{-0.1ex} > 0}$.

\en{And}\ru{И} \en{here}\ru{тут} \en{is assumed that}\ru{предполагается, что} ${\omega^2\hspace{-0.1ex}}$ \en{and}\ru{и}~$\mathboldU$ \en{are real}\ru{вещественны}.
...
\en{In the linear dynamics}\ru{В~линейной динамике} \en{of~rods}\ru{стержней} \en{we have}\ru{мы имеем} \en{the following system}\ru{следующую систему} \en{for}\ru{для} \en{forces}\ru{сил}~$\mathboldQ$, \en{force couples}\ru{пар сил}~(\en{moments}\ru{моментов})~$\mathboldM$, \en{displacements}\ru{перемещений}~$\bm{u}$ \en{and}\ru{и}~\en{rotations}\ru{поворотов}~$\bm{\theta}~(\chapdotpararef{chapter:rods}{para:rods-lineartheory})$:
...
\ru{\section{Волны в упругом континууме}}
