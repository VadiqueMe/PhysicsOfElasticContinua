...
\en{Components}\ru{Компоненты} \en{of the~same vector}\ru{одного и~того~же вектора}~${\bm{v}}$ \en{in}\ru{в}~\en{an~}\inquotes{\en{old}\ru{старом}} \en{and}\ru{и}~\en{a~}\inquotesx{\en{new}\ru{новом}}[---] \en{rotated}\ru{повёрнутом}\:---
\en{orthonormal bases}\ru{ортонормальных базисах}, \en{transformed}\ru{преобразованные}
\en{into each other}\ru{друг в~друга}:
...
${ \bm{e}_{1} \dotp \bm{v}
= \bm{e}_{1} \dotp \bigl( v_{j} \bm{e}_{j} \bigr)
= \bm{e}_{1} \dotp v_{j} \bm{e}_{j} }$
...
\en{The~matrix of~cosines}\ru{Матрица косинусов} \en{is orthogonal}\ru{ортогональна},
\en{meaning that it inverses}\ru{то~есть она обращается}
\en{with }\ru{с} \inquotes{\en{transposing}\ru{транспонированием}}
\hspace{-0.2ex}(\en{swapping}\ru{обменом местами}
\en{of the letters}\ru{букв}
\en{of indices}\ru{индексов})\footnote{\en{More about matrices}\ru{Больше о~матрицах}\:---
\en{in}\ru{в}~\pararef{para:matrices+permutations+determinants}.}:

\nopagebreak\vspace{-0.35em}
\begin{equation}\label{orthogonalityofcosinematrix}
...
\hspace{.1ex} \bm{e}_k
\hspace{-0.15ex} \dotp
\cosinematrix{j'\hspace{-0.1ex}n}
\hspace{.1ex} \bm{e}_n \hspace{-0.2ex}
...
\en{four operations}\ru{четырёх операций}.
\en{This paragraph is about them}\ru{Этот параграф\:--- про них}.

\en{So}\ru{Итак},
\en{the tensor algebra}\ru{тензорная алгебра}
\en{consists of the four operations}\ru{состоит из четырёх операций}.

% linear combination

\nopagebreak\en{The~first}\ru{Первое}\:--- \textbold{\en{linear combination}\ru{линейная комбинация}}\:--- \en{aggregates}\ru{объединяет}
\en{the addition}\ru{сложение}
\en{and}\ru{и}
\en{the multiplication by~number}\ru{умножение на~число}.
...
\en{For the two tensors it looks like this}\ru{Для двух тензоров это выглядит так}:
...
\scalebox{.99}{$
   ...
   B_{pq} \bm{e}_n \bm{e}_m \hspace{-0.2ex}
...
\en{the decomposition into polyads}\ru{разложение по~полиадам},
\en{it’s easy to~see}\ru{легк\'{о} увидеть}
\en{the~actuality}\ru{действительность}
\en{of~property}\ru{свойства}~\eqref{definingpropertyofidentitytensor}\:---
\en{the~}\inquotes{\en{unitness}\ru{единичность}} \en{of~tensor}\ru{тензора}~$\UnitDyad$:
...
\en{Except in rare cases}\ru{Кроме редких случаев},
\en{the direction}\ru{направление}
\en{of a~fully invariant (polar) vector}\ru{полностью инвариантного (полярного) вектора} \en{will change}\ru{поменяется} \en{on a~reflection}\ru{при отражении} \en{together}\ru{вместе} \en{with the entire space}\ru{со всем пространством}.
...
\en{\section{Eigenvectors and eigenvalues. Invariants}}

\ru{\section{Собственные векторы и собственные числа. Инварианты}}
...
= 0 }$
\en{has}\ru{имеет} \en{a~non-zero solution}\ru{ненулевое решение}\ru{,}
\en{if}\ru{если}
...
\vspace{-0.4em}
\en{The roots}\ru{Корни} \en{of characteristic equation}\ru{характеристического уравнения}~\eqref{chardetequation}\:--- \en{eigenvalues}\ru{собственные числа}\:---
\en{don’t depend}\ru{не~зависят}
\en{on the basis}\ru{от~базиса}
\en{and therefore}\ru{и~потому}
\en{are invariants}\ru{суть инварианты}.
...
\en{The first invariant}\ru{Первый инвариант}~${\mathrm{I}}$
...
\vspace{-0.2em}\noindent
\en{And}\ru{И}~\en{the third invariant}\ru{третий инвариант}~${\mathrm{III}}$
\en{is}\ru{это}
