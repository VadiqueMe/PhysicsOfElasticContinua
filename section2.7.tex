...
\en{in the~classical}\ru{в~классической}
\en{generic}\ru{общей}
...
\en{of external forces exerted on it}\ru{от действующих на~него внешних сил}.
...
\en{Here are figured}\ru{Здесь изображены}
\en{the following}\ru{следующие}
\en{triplets}\ru{тройки}
...
\begin{itemize}
   \item
${\mathcircabove{\bm{e}}_i \hspace{-0.2ex} = \hspace{-0.1ex} \boldconstant}$\en{ is}\ru{\:---}
\en{the~}\en{immovable}\ru{неподвижный}~(\en{stationary}\ru{стационарный})
\en{basis}\ru{базис}\footnote{%
...
\en{reference system}\ru{системы отсчёта}.%
   ...
   \item
${\widetilde{\bm{e}}_i}$\en{ is}\ru{\:---}
\en{the~intermediate}\ru{промежуточный}
\en{basis}\ru{базис},
\en{that is}\ru{то есть}
\en{non-rotated}\ru{неповёрнутый}
\en{moving basis}\ru{подвижный базис},
\en{that is}\ru{то есть}
\en{stationary basis}\ru{стационарный базис}~${\mathcircabove{\bm{e}}_i}$
\en{at the~pole}\ru{на полюсе},
${\widetilde{\bm{e}}_i \hspace{-0.2ex} = \mathcircabove{\bm{e}}_i \hspace{-0.2ex} = \hspace{-0.1ex} \boldconstant}$
   \item
${\bm{e}_i}$\en{ is}\ru{\:---}
...
\en{Having}\ru{Имея}
\en{the~}\en{stationary}\ru{стационарный}
\en{basis}\ru{базис}~${\mathcircabove{\bm{e}}_i}$
\en{and}\ru{и}
\en{the~}\en{moving}\ru{движущийся}
\en{basis}\ru{базис}~${\bm{e}_i}$,
\en{the body’s angular orientation}\ru{угловая ориентация тела}
\en{can be}\ru{может быть}
\en{described}\ru{описана}
\en{by the~rotation tensor}\ru{тензором поворота}
${\rotationtensor \equiv \bm{e}_i \widetilde{\bm{e}}_i \hspace{-0.1ex} = \bm{e}_i \mathcircabove{\bm{e}}_i}$\::

\nopagebreak\vspace{-0.6em}
\begin{equation}\label{completelyrigidbody.rotatedwithtensor}
...
%
\bm{x}
= x_i \hspace{.1ex} \bm{e}_i \hspace{.1ex} \widetilde{\bm{e}}_i \hspace{-0.1ex} \dotp \widetilde{\bm{e}}_i \hspace{-0.1ex}
= \bm{e}_i \widetilde{\bm{e}}_i \hspace{-0.1ex} \dotp x_i \hspace{.1ex} \widetilde{\bm{e}}_i \hspace{-0.1ex}
= \rotationtensor \hspace{-0.15ex} \dotp \hspace{.1ex} \widetilde{\bm{x}}
\hspace*{1em}
\end{equation}

\nopagebreak\vspace{-0.5em}\noindent
\eqrefwithchapterdotsection{rodriguesrotationformula}{chapter:mathapparatus}{section:rotationtensors},
\en{and}\ru{а}~%
${\widetilde{\bm{x}} = \mathcircabove{\bm{x}} = x_i \hspace{.1ex} \mathcircabove{\bm{e}}_i = \boldconstant}$\en{ are}\ru{\:---}
\en{known locations}\ru{известные положения}
\en{of body points}\ru{точек тела}.
%
\en{For}\ru{Для}
\en{a~}\en{non-deformable}\ru{недеформируемого}
\en{rigid}\ru{жёсткого}
\en{body}\ru{тела}
\en{not only}\ru{не только}
\en{vector}\ru{вектор}~${\mathcircabove{\bm{x}}}$\en{ is}
\en{constant}\ru{постоянен},
\en{but also}\ru{но также}
...
\en{can be}\ru{может быть}
\en{completely determined}\ru{полностью определено}
\en{by the~two functions}\ru{двумя функциями},
${\positionofthepole(t)}$
\en{and}\ru{и}~${\rotationtensor(t)}$

\nopagebreak\vspace{-0.5em}
\begin{equation}\label{completelyrigidbody.locationvectorofsomepoint}
\locationvector = \positionofthepole + \bm{x}
= \positionofthepole + \rotationtensor \hspace{-0.15ex} \dotp \hspace{.1ex} \mathcircabove{\bm{x}}
\end{equation}

\nopagebreak\vspace{-0.5em}\noindent
---
\en{the~}\en{location vector}\ru{вектор положения}
\en{of~some body’s point}\ru{некоторой точки тела}.

\en{Differentiating}\ru{Дифференцируя}
\en{by time}\ru{по времени},

\nopagebreak\vspace{-0.4em}
\begin{equation}\label{completelyrigidbody.locationvectortimederivative}
\mathdotabove{\locationvector} = \mathdotabove{\positionofthepole} + \mathdotabove{\bm{x}}
= \mathdotabove{\positionofthepole} \hspace{.1ex} + \hspace{-0.1ex} \bigl( \rotationtensor \hspace{-0.15ex} \dotp \hspace{.1ex} \mathcircabove{\bm{x}} \hspace{.1ex} \bigr)^{\timederivativedot} \hspace{-0.1ex}
= \mathdotabove{\positionofthepole} \hspace{.1ex} + \mathdotabove{\rotationtensor} \hspace{-0.15ex} \dotp \hspace{.1ex} \mathcircabove{\bm{x}}
\hspace{.1ex} .
\end{equation}

\vspace{-0.5em}\noindent
\textcolor{magenta}{\en{But}\ru{Но}
\en{what is}\ru{что такое}~${\mathdotabove{\rotationtensor}}$?}
\\
(${x_i \hspace{-0.2ex} = \hspace{-0.1ex} \constant
\hspace{.6ex} \Rightarrow \hspace{.8ex}
\mathdotabove{\bm{x}} = x_i \hspace{.1ex} \mathdotabove{\bm{e}}_i \hspace{-0.1ex}
= x_i \hspace{.1ex} \mathdotabove{\bm{e}}_i \mathcircabove{\bm{e}}_i \hspace{-0.2ex} \dotp \hspace{-0.1ex} \mathcircabove{\bm{e}}_i \hspace{-0.1ex}
= \mathdotabove{\rotationtensor} \hspace{-0.15ex} \dotp x_i \hspace{.1ex} \mathcircabove{\bm{e}}_i \hspace{-0.1ex}
= \mathdotabove{\rotationtensor} \hspace{-0.15ex} \dotp \hspace{.1ex} \mathcircabove{\bm{x}}}$,
\en{where}\ru{где}
${\mathdotabove{\bm{e}}_i \mathcircabove{\bm{e}}_i \hspace{-0.1ex} = \mathdotabove{\rotationtensor}}$,
...
\eqrefwithchapterdotsection{angularvelocityvector}{chapter:mathapparatus}{section:rotationtensors}

\nopagebreak\vspace{-0.5em}
\begin{equation}\label{completelyrigidbody.introductionofangularvelocity}
\mathdotabove{\rotationtensor} \hspace{-0.1ex}
= \bm{\omega} \hspace{-0.2ex} \times \hspace{-0.2ex} \rotationtensor
, \hspace{1.2ex}
\mathdotabove{\bm{e}}_i \hspace{-0.16ex}
= \bm{\omega} \hspace{-0.2ex} \times \hspace{-0.2ex} \bm{e}_i
\hspace{.1ex} , \hspace{1.2ex}
\mathdotabove{\bm{x}} = \bm{\omega} \hspace{-0.2ex} \times \hspace{-0.2ex} \bm{x}
\end{equation}

....

\en{The second time derivative}\ru{Вторая производная}
\en{is}\ru{есть}
\en{acceleration}\ru{ускорение}
...
\en{Three}\ru{Три}
\en{inertial characteristics}\ru{инерциальных характеристики}
\en{of the~body}\ru{тела}\::
...
\vspace{.2em}
\item \en{eccentricity vector}\ru{вектор экцентриситета}\hbox{~\hspace{.2ex}}$\bm{\Xi}$\:--- \en{measures}\ru{измеряет} \en{the~offset}\ru{смещение} \en{of the chosen pole}\ru{выбранного полюса}
\en{from}\ru{от}
\en{the body’s }{\inquotes{\en{center of mass}\ru{центра масс}}}\ru{ тела},
%%\en{and}\ru{и}
\vspace{.2em}
...
.... \eqref{completelyrigidbody.locationvectorofsomepoint} ${\Rightarrow}$
